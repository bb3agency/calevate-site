...
export function StickyHeader({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-[14px] border border-line bg-surface shadow-[0_1px_2px_rgba(28,25,23,0.06),0_4px_10px_-8px_rgba(28,25,23,0.45)] dark:shadow-[0_1px_6px_rgba(0,0,0,0.45)] ${className}`}
    >
      <div
        ref={ref}
        ...
        className="overflow-y-auto overscroll-y-contain outline-none [scrollbar-gutter:stable] focus-visible:bg-brand/[0.06] focus-visible:shadow-[inset_0_0_0_1px_#16a05d] dark:focus-visible:bg-brand-bright/[0.06] dark:focus-visible:shadow-[inset_0_0_0_1px_#22c55e]"
      >
        ...
        <div
          aria-hidden
          className="pointer-events-none sticky bottom-0 -mt-6 h-6 bg-gradient-to-t from-surface to-transparent"
        />
      </div>
      <header
      ...
      >
        <motion.div
          ...
          className="absolute inset-x-0 top-0 origin-top bg-surface"
        />
        <motion.div
          aria-hidden
        ...
        <motion.div
          aria-hidden
          style={{ y: edge, opacity: lifted }}
          className="absolute inset-x-0 top-0 h-5 bg-gradient-to-b from-surface to-transparent"
        />
        <motion.div
          aria-hidden
          style={{ y: edge, opacity: lifted }}
        ...
        <div className="absolute inset-x-0 top-0 flex items-start gap-2.5 px-4 pt-3">
          ...
          <div className="relative min-w-0 flex-1">
            ...
            >
              <h2 className="truncate text-[20px] font-medium leading-[1.2] tracking-[-0.03em] text-ink">
                {title}
              </h2>
              {subtitle ? (
                <p className="mt-0.5 truncate text-[11.5px] leading-[1.35] text-ink-muted">
                  {subtitle}
                </p>
              ) : null}
            </motion.div>
            <motion.span
              ...
              className="absolute inset-x-0 top-[3px] truncate text-[13px] font-medium leading-[1.4] text-ink"
            >
              {title}
            </motion.span>
          </div>
