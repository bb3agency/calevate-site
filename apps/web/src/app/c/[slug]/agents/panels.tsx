...
import { useSetDisclosure, type Agent, type AgentExtractionField } from "@/lib/api/agents";
import {
  usePendingChanges,
  type PendingChange,
  type PendingState,
...
/**
 * What the agent writes down — the CRM columns this agent's calls fill in.
 *
 * `leadsHref` is passed rather than derived so this component knows nothing about routing
 * or about the view-as marker; the screen that has the slug builds the link.
 */
    ...
    <section>
      ...
      {agent.extraction_fields.length > 0 ? (
        <>
          ...
          <p className="mt-2 text-xs text-ink-muted">
            These are the columns in your {leadsHref} table. The agent fills them in from
            the conversation — it never reads a form aloud, so a caller who answers early is
            not asked twice.
            {hasRequired && (
              <>
                {" "}
                {/* What `required` actually does: it marks the field REQUIRED in the
                    extraction instruction and, when the call ends without it, the value is
                    dropped and the extraction is recorded as incomplete
                    (packages/shared/.../extraction.py). A badge reading "Always asked"
                    would promise an interrogation this product deliberately does not do —
                    and would contradict the sentence right above it. */}
                The ones marked <span className="font-medium text-ink">Required</span> are
                what the agent is told to capture on every call; a call that ends without
                one still becomes a lead, with that column left empty.
              </>
            )}
          </p>
        </>
        ...
        <p className="mt-2 text-sm text-ink-muted">
          Nothing extra yet. Calls still turn into leads with the caller name, number and a
          summary — this agent just does not capture any business-specific details on top
          of that.
        </p>
      )}
      <p className="mt-2 text-xs text-ink-muted">
        The column list is set up with you by your account manager: changing it regenerates
        what the agent is told to listen for and needs a test run against real calls before
        it goes live (D-21).
      </p>
    </section>
  );
...
/**
 * One captured field, shown as the CRM column it becomes. The label leads, because that is
 * the word the client sees on the Leads table; the key is not shown at all — it is our
