...
believes the round trip works. OPERATIONS §2 gate 16(a) is what closes it.
...
class CartesiaEngine:
    ...
    def _agent_body(self, cfg: AgentConfig) -> dict[str, Any]:
        ...
        **CONTRADICTED IN PART, AND THE PART THAT IS CONTRADICTED IS THE COMPLIANCE HALF
        ...
        OPERATIONS §2 gate 16(a). Against a real account they will be ignored or rejected,
    ...
    async def create_agent(self, cfg: AgentConfig) -> EngineAgentRef:
        ...
        WHAT THE REAL PATH LOOKS LIKE, for whoever closes gate 16(a): an agent already
    ...
    async def update_agent(self, ref: EngineAgentRef, cfg: AgentConfig) -> None:
        ...
        on Cartesia is the deployed program, which is outside this repository. Gate 16(a).
        """
    ...
    # is a hint that ingestion lives somewhere else entirely. Gate 16(f).
    #
    ...
    def _cost(self, payload: dict[str, Any]) -> CostBreakdown | None:
        ...
        * `cartesia-mcp/cartesia_mcp/extra_api.py:54-79` — usage is an ACCOUNT-level meter,
        ...
        Scale tier — not an endpoint anybody can go and find. Gate 16(c).
        """
        return None
    ...
    def _snapshot(self, payload: dict[str, Any]) -> ExecutionSnapshot:
        ...
        # maps straight through rather than inventing a swap. Gate 16(d).
        ...
        return ExecutionSnapshot(
            ...
            # holds no engine credential. A URL that 401s is worse than no URL. Gate 16(b).
    ...
    async def get_execution(self, call_id: str) -> ExecutionSnapshot:
        ...
        guessing a parameter onto a path that does not declare one. Gate 16(b).
