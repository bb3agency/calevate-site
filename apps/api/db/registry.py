...
RLS_EXEMPT_TENANT_COLUMNS = {
    ...
    "engine_agent_routes": (
        "inbound routing table, and THE EXEMPTION IS FOR READS AND ONLY READS. An engine "
        ...
        "(FOR SELECT USING (<guc> IS NULL)) is what keeps that working. THAT ARM IS THE "
        "UNTENANTED SESSION AND ONLY IT since migration d7c2f4a91b83: it was "
        "`USING (true)`, which — permissive policies being OR'd per command — let a "
        "session scoped to tenant A READ tenant B's routes (measured as `calevate_app`, "
        "14 Sep 2026: `count = 1` on a neighbour's row). Nothing in this entry ever "
        "argued for that; every question it names is one no tenant session can ask. "
        "EVERY WRITE VERB IS "
        "POLICIED AGAINST THE ROW'S OWN TENANT: since migration b8e2d47f0c19 the FORCEd "
        "`tenant_isolation` policy is the strict repo-wide form `tenant_id = <guc>` for "
        "USING and WITH CHECK alike, so a session with NO `app.tenant_id` — ops, a "
        "worker, a sweep — can read every row here and write none of them. IT USED TO "
        "CARRY `OR <guc> IS NULL` ON BOTH, which handed every untenanted writer, present "
        "and future, INSERT/UPDATE/DELETE over any tenant's row (measured as "
        "`calevate_app`: an untenanted INSERT naming an arbitrary tenant returned "
        "`INSERT 0 1`), on the table that decides WHICH AGENT AN INBOUND NUMBER REACHES. "
        "c4b70e928a1f wrote that arm down as deliberate because the drift sweeps used "
        "it; what closed it is that they never needed it. The sweeps' cross-tenant leg "
        "is the BATCH READ (`claim_drift_batch`/`claim_kb_drift_batch`), which the "
        "global-read policy buys and which is still untenanted, and every candidate it "
        "returns already names its tenant — so `record_drift`/`record_kb_drift` stamp "
        "`drift_state`/`kb_drift_state` under `tenant_session(candidate.tenant_id)`, the "
        "same per-tenant re-scoping `dispatch_scan` (a8d4f21c9b06) chose over a "
        "privileged role. Before c4b70e928a1f a session scoped to tenant A could also "
        "DELETE, deactivate or RE-TENANT tenant B's inbound route — the same bug "
    ...
    "engine_kb_routes": (
        ...
        "(FOR SELECT USING (<guc> IS NULL), narrowed from `USING (true)` by migration "
        "d7c2f4a91b83 — the wider form let a tenant session read a neighbour's claim, "
        "which no sentence here asks for) beside a FORCEd `tenant_isolation` policy "
        "covering INSERT/UPDATE/DELETE, so one client's session can neither read, delete "
        "nor re-tenant another's claim. THAT WRITE POLICY IS `tenant_id = <guc>` AND NOTHING ELSE "
        "since migration b8e2d47f0c19: it carried `OR <guc> IS NULL` too, which let any "
        "untenanted session insert, re-tenant or delete any claim here (measured), and "
        "unlike `engine_agent_routes` this table never had an untenanted writer to lose "
        "— every write is `kb/service.record_engine_kb_ref`, reached only from "
        "`publish_source`, whose two callers (`admin/routes.publish_kb`, "
        "`workers/kb_ingest`) both open `tenant_session` first. The read genuinely is "
        "global: we run ONE engine account for "
    ...
    "pipecat_kb_objects": (
        ...
        "(FOR SELECT USING (<guc> IS NULL), narrowed from `USING (true)` by migration "
        "d7c2f4a91b83 for `engine_kb_routes`' reason) beside a FORCEd `tenant_isolation` "
        "policy covering INSERT/UPDATE/DELETE, so one client's session can neither read, "
        "delete nor re-tenant another's object. The UNTENANTED read genuinely is needed, "
    ...
    "platform_voice_catalog": (
        ...
        "of these could be and it carries no tenant_id. Written by the voice-catalogue sync "
        "(`agents/voice_sync.py`, an ARQ cron plus an ops refresh) and — since D-588 — by "
        "the operator CURATION write (`agents/voice_curation.set_curation_state`, behind "
        "`PATCH /v1/ops/voices` on `ops:manage`, audited in the same transaction). Read "
        "through `agents/voice_offer.offered_catalogue`, which deliberately answers the "
        "same catalogue to every tenant. Mostly a cache, NOT a ledger: refreshed whole and "
        "not in APPEND_ONLY_TABLES. "
        "⚠ ONE COLUMN IS NOT RE-DERIVABLE and that changes what a DELETE here means: "
        "`curation_state` is an operator's decision about what this platform sells, not a "
        "copy of the vendor's list, which is why a voice the vendor stops listing is "
        "stamped `withdrawn_at` rather than deleted — a delete would silently un-curate a "
        "voice that came back. Nothing here is still PII, a credential or tenant data."
    ),
    "platform_tts_prices": (
