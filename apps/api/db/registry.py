...
RLS_EXEMPT_TENANT_COLUMNS = {
    ...
    "engine_agent_routes": (
        ...
        "AGAINST A TENANT-SCOPED SESSION ONLY, and the distinction is the whole content "
        "of the entry: migration c4b70e928a1f added a FORCEd `tenant_isolation` policy "
        "covering INSERT/UPDATE/DELETE whose expression is `tenant_id = <guc> OR <guc> "
        "IS NULL`, so it constrains a session that carries `app.tenant_id` and lets an "
        "UNTENANTED one write ANY row. That arm is deliberate (the same asymmetry "
        "a1c8e40f27b9 gave `dnc_list`) and it is not a corner case here: the drift "
        "sweeps that stamp `drift_state`/`kb_drift_state` across tenants run from "
        "`untenanted_session` by design, so the untenanted leg is the one the table's "
        "own background writers use, while the publish path writes under the tenant. "
        "The claim this comment may make, therefore, is the narrow one — A CLIENT "
        "REQUEST CANNOT REACH ANOTHER TENANT'S ROUTE, because `tenant_session` is the "
        "only way a client request reaches the database. It USED TO SAY, flatly, that "
        "writes are policied; that reads as a claim about every writer and is false of "
        "the ones that actually run. Before the migration a session scoped to tenant A "
