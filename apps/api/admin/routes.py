...
class EditTenantOut(BaseModel):
    ...
    #: retarget is SAID rather than discovered (D-546).
    #:
...
async def edit_tenant(
    ...
    **WHAT WAS FOUND ABOUT NOTICES ALREADY IN THE QUEUE (D-546), STATED RATHER THAN
...
class TenantProfileOut(BaseModel):
    """A client's business record as the correction form reads it back (D-546).
...
# columns together.
#
# ⚠ **`churned` IS NO LONGER A TARGET EITHER (D-546, 6 Sep 2026), AND THIS TABLE USED TO
...
class LifecycleIn(BaseModel):
    ...
    #: TWO MEMBERS, NOT THREE (D-546). `churned` was removed from the Literal rather than
...
async def set_tenant_status(
    ...
    **NO STEP-UP HERE ANY MORE, AND THAT IS BECAUSE THE DANGEROUS MOVE LEFT** (D-546).
