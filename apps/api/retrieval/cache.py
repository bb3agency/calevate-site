...
you take walk-ins, what does X cost — so the same normalised question recurs all day, and a
repeat that never reaches the store is the only headroom this design can buy without
knowing who wins the D-28 bake-off.

**THAT HEADROOM IS FOR THE DASHBOARD LEG. IT DOES NOT MAKE IN-CALL RETRIEVAL AFFORDABLE,
AND AN EARLIER DRAFT OF THIS PARAGRAPH IMPLIED IT DID.** It cited TRD §6.2's 100ms and
D-109's p95 1.4ms server half and concluded that what remained was "the budget to the
network and the store". The arithmetic that matters is one level up and says the opposite:
`calevate_shared.engine.latency_budget_composes()` is **False**, and
`voice_to_voice_gap_ms()` is **+100.0** — the declared pipeline floor MISSES the headline
target by 100ms — with `RETRIEVAL_BUDGET_MS = 100.0` ALREADY ALLOCATED INSIDE that floor
(`engine.py:3040,3114-3122`; `packages/shared/tests/latency_budget_test.py:113-114` pins
both). So there is no unspent room for an in-call retrieval to move into; the budget is
overspent before one is added. **In-call stays T0 because there is nothing to spend, not
because a store is slow** — which is also why no cache hit rate can rescue it, and why
`retrieval/__init__.py` mounts no in-call route.
...
The cost of that choice, stated plainly and now MEASURED rather than estimated: the epoch
read happens BEFORE the cache lookup, so a hit is one small indexed Postgres read plus one
Redis round trip, not a Redis round trip alone. Both halves are measured separately in
`tests/retrieval_latency_test.py` — 31 Aug 2026, n=60, contended development container —
and the split is lopsided: **Redis p50 0.15ms, whole hit p50 2.61ms**, so ~2.4ms of a hit
is the epoch read.

**AND THE HIT IS SLOWER THAN THE MISS AT T0: 2.61ms against 2.34ms.** The T0 "store" is
itself one indexed read on the same Postgres, so this cache currently replaces a cheap read
with a cheaper read plus a round trip. That is recorded here rather than buried because it
is the honest state of the tier and because it does not make the cache wrong — what the
cache buys at T0 is the SHAPE (per-tenant keyspace, epoch-stamped correctness, a bounded
TTL) that a REMOTE provider will need the day the D-28 bake-off names one, and that is when
the miss leg becomes expensive and the arithmetic inverts. What it must not be described as,
until a bake-off winner makes it true, is a latency win.
...
async def invalidate_tenant(tenant_id: UUID) -> int:
    ...
    NOT the ordinary invalidation path — the epoch is, and it needs no caller. Every routine
    case is already covered by the stamp: a knowledge publish mints a new prompt version, and
    an agent going live, being paused or being archived moves the COUNT half of the epoch.

    ⚠ **NO PRODUCTION CODE CALLS THIS TODAY, AND THAT IS STATED RATHER THAN IMPLIED.** Its
    two intended callers do not exist yet: there is no tenant-purge path in this tree (a DPDP
    erasure is per-SUBJECT — `compliance/deletion.request_erasure` takes a phone number — and
    a subject's personal data cannot be in this keyspace anyway, because `normalise` REFUSES
    to key a question `redact` would change), and there is no operator route for "clear a bad
    answer NOW". What does call it is the retrieval test suite, as teardown, which is a real
    use: without it every run leaks keys into a shared Redis. It is kept rather than deleted
    because it is the correct implementation of a scan that must never be written twice and
    must never be written as `KEYS`, and because the day either caller lands it is this
    function they call. If a third session finds it still uncalled, that is the signal to
    delete it, not to invent a route for it.
