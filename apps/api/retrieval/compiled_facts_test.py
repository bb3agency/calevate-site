...
def test_the_shortest_price_question_finds_the_line_that_answers_it() -> None:
    """THE REGRESSION THIS TIER SHIPPED WITH. "what does it cost" reduces to the single
    content token `cost`; the fact line says `costs`; exact overlap was empty and the tier
    returned NOTHING on the commonest question an SMB's caller asks. `_singular` folds the
    one inflection, symmetrically on both sides.

    FAILS IF: somebody removes the fold as "stemming the module docstring refuses". The
    integration tests then go red too, which is the point — this one names the reason."""
    question = tokens("what does it cost")
    assert question == frozenset({"cost"})
    answer = "- Fees: A consultation costs 500 rupees."
    assert score_line(question, answer) > 0.0


def test_the_fold_cannot_merge_two_different_words() -> None:
    """The bound on the rule: it drops one trailing `s`, never a syllable. `address` and
    `business` end in `ss` and survive whole, so no question about an address can be
    answered out of a line about anything ending in `addres`."""
    assert tokens("address") == frozenset({"address"})
    assert tokens("business") == frozenset({"business"})
    # Symmetric: the question and the line fold identically, so a match means one word.
    assert tokens("hours") == tokens("hour")
    assert tokens("prices") == tokens("price")


def test_the_fold_runs_after_stop_words_not_before() -> None:
    """`does` ends in `s`. Folding first would make it `doe` — not in the stop list, and
    therefore a content word ranking on every question that contains "does"."""
    assert "doe" not in tokens("what does it cost")
