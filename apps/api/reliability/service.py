...
async def mark_inbox_processed(session: AsyncSession, *, row_id: UUID) -> None:
    """Close the inbox row — under a CAS — and record how long the deferred half took.

    TWO CHANGES THAT ARRIVED SEPARATELY AND BOTH BELONG. The CAS (D-321) is the
    correctness half: this was a blind UPDATE, and the race is one this module
    DELIBERATELY creates — `claim_inbox` hands a lapsed claim to a second holder by
    design, so two callers legitimately hold one row and the first is by definition late.
    Without the guard a late writer closed a claim the new holder owned, and the next
    claim answered `fresh` where it owed `replay` — which on the post-call path is the
    pipeline re-driven for a metered call. `mark_outbox_failed` already guarded on
    `status = 'pending'` and argued at length that the guard was load-bearing; these four
    terminal writers simply had not been brought into line with it.

    The measurement is the observability half: `processed_at` was written here and read
    by nothing, on the one row that knows both when a webhook arrived and when its work
    finished, in a system whose only latency number was the 500ms ACK the caller sees
    (hard rule 3). A fast ack over a slow pipeline is exactly the shape that looks
    healthy from outside, so the two are measured separately and by name.
    ...
    extra round trip on the post-call path. It also does the CAS's work for free — no row
    returned IS the lost race, so the guard costs nothing beyond the predicate.
    """
    row = (
        await session.execute(
            text(
    ...
    if row is None:
        # The CAS lost: this caller's claim had already lapsed and another holder owns the
        # row. Nothing to record — the metric belongs to whoever actually closed it.
