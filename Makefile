...
guardrails:  ## Executable governance (ENGINEERING-PRACTICES.md §2); grows per milestone
	...
	# The obligation with the highest price and, until now, the only hard-rule-adjacent
	# surface with no enumeration gate: DPDP erasure. Every live table carrying a
	# subject handle, a link to a call or a lead, or a free-text/jsonb payload is named
	# by an erasure arm or registered in ERASURE_EXEMPT with a reason. Needs the same
	# migrated database as the two checks above it. Negative controls, including the
	# doctored states it must fail on, in tests/erasure_coverage_guard_test.py.
	uv run python -m scripts.check_erasure_coverage
	# Every vendor that CAN receive client data is named on /legal/subprocessors, derived
	# from the code (credential-shaped Settings fields and the adapter modules) rather than
	# from the register, which is the side that drifts: three vendors were live and
	# unpublished while apps/web's own legal test passed, because that test compares the
	# register with itself. Both directions, and it needs no database.
	uv run python -m scripts.check_subprocessor_coverage
