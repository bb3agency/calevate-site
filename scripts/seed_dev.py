...
async def _run() -> str:
    ...
    return "\n".join(
        [
            ...
            "  SECOND FACTOR: the admin realm needs an emailed six-digit code (D-170),",
            "  and it is NOT bypassed on local — you type the real code, as an operator",
            "  would. Locally it is delivered to a terminal by the console sink:",
            "",
            "    *** THE WORKER MUST BE RUNNING ***  the mail is sent by the",
            "    `deliver_auth_email` ARQ job, so the code appears in the WORKER's log,",
            "    not the API's:  uv run arq apps.workers.settings.WorkerSettings",
            "",
            "    Then grep it out:  ... | grep dev_message",
            "",
            "  (This note used to say the API server printed it. Both halves were wrong:",
            "  the sink logged only a character count, and the sender is the worker.)",
            "",
