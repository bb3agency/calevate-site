...
development container with the shared Postgres (5433) and Redis (6380). Read off a real run
of this test; re-run with `-s` to reproduce. A CONTENDED BOX, so these are upper bounds.

    | leg                                   | p50 (ms) | p95 (ms) |
    |---------------------------------------|----------|----------|
    | cache.get alone (Redis round trip)    |   0.15   |   0.17   |
    | look_up hit (epoch read + cache.get)  |   2.61   |   3.04   |
    | look_up miss (epoch read + store)     |   2.34   |   2.77   |

**WHAT THAT TABLE SAYS, AND IT IS NOT WHAT THE DESIGN EXPECTED.** The Redis leg is 0.15ms
and the hit is 2.61ms, so ~2.4ms of every hit is THE EPOCH READ — it is not a rounding
error on top of a cache lookup, it is essentially the entire cost of one. And because the
T0 store is itself one small indexed read on the same Postgres, the hit (2.61) costs MORE
than the miss it replaces (2.34): **at T0 this cache does not pay for itself in latency.**
That is a real result, not a flaky run — it reproduces, and it is the number any future
provider decision should be compared against, since a remote store is where the miss leg
...
async def test_the_hit_path_is_measured_and_reported(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    # THE ONE RELATIONSHIP THAT IS A PROPERTY AND HOLDS ON ANY MACHINE AT ANY SPEED: the
    # hit path CONTAINS the Redis leg, so it cannot be faster than it. Everything else in
    # this file is a measurement, and measurements are printed rather than asserted.
    assert redis["p50"] <= whole["p50"], (redis, whole)

    # ⚠ `hit <= miss` WAS ASSERTED HERE AND IT IS NOT TRUE FOR T0. It read as a property —
    # "a cache hit cannot cost more than the store read it replaces" — and it is really a
    # hypothesis about the STORE: it holds when the store is a remote vector service and
    # fails when the store is one small indexed read on the same Postgres, which is exactly
    # what T0 is. Measured here (31 Aug 2026, development container, shared Postgres 5433 /
    # Redis 6380, n=60): hit p50 2.61ms vs miss p50 2.34ms, with the Redis leg itself only
    # 0.15ms — so what the hit really pays for is the EPOCH READ, not Redis. The cache is
    # not earning its keep on latency at T0 and this file must not claim it is. What it IS
    # earning is stated in `cache.py`:
    # a bounded, per-tenant, epoch-stamped answer that a REMOTE provider will need the day
    # the D-28 bake-off names one, and the number above is the baseline that decision gets
    # compared against. Asserting the false ordering would have made an honest measurement
    # into a red test on the machine that produced it.
