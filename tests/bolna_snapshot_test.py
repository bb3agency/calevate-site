...
1. **The currency.** `source_currency="USD"` was a literal, so reading it back was the
   harness agreeing with itself. Bolna publishes no OpenAPI spec — "costs arrive in USD
   cents" is a claim off their docs, and it is worth the exchange rate. Every INR row in
   `usage_events` inherits it (hard rule 7).
...
from apps.api.engine.bolna import BolnaEngine, parse_transcript
