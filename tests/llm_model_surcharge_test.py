...
async def test_a_month_that_straddles_a_model_switch_totals_exactly() -> None:
    ...
    assert to_paise(
        calling_revenue_inr(
            ...
            # MANAGED, so what the wallet was charged is not read at all: this motion is
            # invoiced against a retainer and takes no lot debit. A number nothing else in
            # this file uses, so a managed answer that ever moved with it would be
            # unmistakable.
...
def test_the_month_total_is_the_three_published_components_and_nothing_else(
    ...
    answer = month_charges_inr(
        ...
        calling_inr=calling_revenue_inr(
            ...
            # MANAGED: the wallet is not read on this branch — there is no wallet.
            prepaid_charged_inr=Decimal("999.00"),
        ),
    )
    assert answer == Decimal(expected)
...
def test_the_client_total_and_the_admin_revenue_are_the_same_expression() -> None:
    ...
    calling = calling_revenue_inr(
        plan_tier="managed",
        overage_cost_inr=overage,
        llm_surcharge_inr=surcharge,
        prepaid_charged_inr=Decimal("999.00"),
    )
    assert month_charges_inr(monthly_fee_inr=fee, calling_inr=calling) == fee + calling
