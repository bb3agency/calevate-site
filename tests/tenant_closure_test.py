...
async def _patch(
    ...
    """`confirm=True` sends the step-up header a NOTICE-ADDRESS change now requires (D-546).
...
async def test_the_edit_cannot_reach_a_field_with_its_own_screen() -> None:
    """`extra="forbid"` and a THREE-field whitelist (D-546 added the vertical). A
...
# ═══════════════════ D-546: the rest of the business record ═══════════════════
#
...
async def test_a_notice_already_queued_follows_the_address_and_the_edit_says_so() -> None:
    """THE FINDING D-546 REPORTS RATHER THAN CHANGES.
...
async def test_the_profile_read_answers_what_the_form_edits_and_no_more() -> None:
    """The read the console had to grow (D-546): `GET /v1/admin/tenants/{id}` is the
