...
4. **`churned` is terminal ON THIS SURFACE**, so a status flip cannot un-end an
   ...
   ⚠ **AND SINCE D-546 IT CANNOT BE REACHED FROM THIS ROUTE AT ALL.** `LifecycleIn.status`
...
async def _set_status(token: str, tenant_id: UUID, status: str, reason: str | None = None) -> Any:
    ...
    It existed because the terminal move lived here and demanded a confirmation; D-546
...
async def _close(token: str, tenant_id: UUID, reason: str) -> Any:
    """Close an account the ONE way the product now offers (D-546).
    ...
    async with tenant_session(tenant_id) as session:
        await session.execute(
            text(
                ...
                "'outbound', 'Idi AI assistant. Call record avutundi.', 'Idi AI assistant. Call "
                "record avutundi.', 'This call is being recorded.', 'I keep a short note of "
                "what you ask about.', 'live', 'fake', now(), now())"
            ),
...
async def test_this_route_cannot_close_an_account_at_all() -> None:
    """THE D-546 COLLAPSE, pinned at the API.
...
async def test_the_status_route_still_cannot_reopen_a_closed_account() -> None:
    """The other direction, unchanged by D-546 and worth keeping asserted.
...
async def test_suspending_still_needs_no_second_factor() -> None:
    ...
    ⚠ **IT IS NOW THE WHOLE OF THIS ROUTE'S POSTURE, NOT HALF OF A SPLIT** (D-546). It used
