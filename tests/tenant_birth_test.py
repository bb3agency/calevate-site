...
async def _set_status(token: str, tenant_id: UUID, status: str, reason: str | None = None) -> Any:
    """Suspend or reactivate. This route can no longer close an account (D-545).
...
async def _founder(email: str) -> str:
    ...
    async with untenanted_session() as session:
        await session.execute(
            text(
                # `email_verified_at` IS SET, because `signup.assert_email_verified`
                # now requires a proved mailbox before a tenant is created. This
                # helper stands in for a person who has completed that step; the
                # gate itself is asserted in `tests/self_serve_test.py`.
                "INSERT INTO users (id, email, email_verified_at, created_at, "
                "updated_at) VALUES (:i, :e, now(), now(), now())"
            ),
            {"i": user_id, "e": email},
        )
...
async def test_churning_a_client_leaves_its_data_on_the_retention_clock(s3: FakeS3) -> None:
    ...
    /v1/admin/tenants/{id}/closure`, which since D-545 is the only way an account is ever
