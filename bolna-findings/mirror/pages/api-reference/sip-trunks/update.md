...
````yaml PATCH /sip-trunks/trunks/{trunk_id}
openapi: 3.1.0
info:
  title: Bolna API
  description: >-
...
paths:
  /sip-trunks/trunks/{trunk_id}:
    patch:
      description: >-
        Partially update an existing SIP trunk. Only included fields are
        changed. Gateways and ip_identifiers arrays are fully replaced when
        provided.
      parameters:
        - in: path
          name: trunk_id
...
components:
  schemas:
    SIPTrunkUpdateRequest:
      type: object
      description: >-
        Fields to update on an existing SIP trunk. Only provided fields are
        changed.
      properties:
        name:
          type: string
          description: Updated human-readable trunk name.
        description:
          type: string
          description: Updated trunk description for internal reference.
        auth_username:
          type: string
          description: >-
            Updated SIP username (used only when the trunk uses userpass
            authentication).
        auth_password:
          type: string
          description: >-
            Updated SIP password (used only when the trunk uses userpass
            authentication).
        gateways:
          type: array
          description: Replaces existing gateways when provided.
          items:
            $ref: '#/components/schemas/SIPTrunkGateway'
        ip_identifiers:
          type: array
          description: Replaces existing IP identifiers when provided.
          items:
            $ref: '#/components/schemas/SIPTrunkIPIdentifier'
        allow:
          type: string
          description: Comma-separated codecs to allow (e.g., "ulaw,alaw").
        disallow:
          type: string
          description: Comma-separated codecs to disallow (typically "all").
        inbound_enabled:
          type: boolean
          description: Enable or disable inbound calling on this trunk.
        ...
        is_active:
          type: boolean
          description: >-
            Activate or deactivate the trunk. When false, calls will not be
            placed.
        qualify_frequency:
          type: integer
          description: SIP OPTIONS ping interval in seconds. Use 0 to disable.
        transport:
          type: string
          enum:
            - transport-udp
        ...
        media_encryption:
          ...
          enum:
            - 'no'
            - sdes
          description: RTP encryption. "sdes" requires transport=transport-tls.
