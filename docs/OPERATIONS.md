...
> gate 14 below rather than a footnote here, because a re-search on 16 Aug 2026 moved it
> from "unverified" to "the evidence points the other way".
> `GEMINI_MODEL_CONFIRMED_IN_REGION is False`.
...
| 14 H | **Does `asia-south1` serve our Gemini model?** [NEW, 16 Aug 2026] | **Not Bolna's account — ours, and it needs nothing from the pilot but a GCP project.** `GEMINI_MODEL_CONFIRMED_IN_REGION is False`: nobody in this repository has been able to read Google's model-availability table (`docs.cloud.google.com`, `firebase.google.com`, `discuss.google.dev`, `promptfoo`, `modelavailability.com` and `litellm` are all refused by the build environment's egress proxy), and the 16 Aug re-search made the picture WORSE rather than merely unresolved: independent summaries place `gemini-3.1-flash-lite` on the GLOBAL endpoint and the `us`/`eu` multi-region REP endpoints, one reports it absent even from `us-central1`, and what they DO place in Mumbai is the 2.5 Flash / 2.5 Flash-Lite class — **the family that retires 16 Oct 2026** (BRD R-04). So this gate has a calendar on it. **The test is one call**: `POST` `vertex_generate_url(project, GEMINI_DEFAULT_LLM)` with a service-account bearer. **Pass** = a 200, and the flag is raised in `calevate_shared/engine.py`. **Fail** = a 404, which the worker logs as `vertex_model_not_served_in_region` naming the region, the model and the flag; the fix is then to record WHICH Gemini `asia-south1` does serve (walk the model list downward from 3.x) and change `GEMINI_DEFAULT_LLM` to it. **What is NOT an acceptable outcome, and this is the whole of why the gate is H:** widening the region, or putting `locations/global` in the path. Google states you cannot control or know which region processes a global-endpoint request — the same sentence that disqualifies AI Studio — and `scripts/check_model_residency.py` fails the build on both. If NO Gemini serves `asia-south1` by 16 Oct 2026, D-127's model leg reopens as a decision (Sarvam serves the dashboard assistant with a disclosed fallback meanwhile, so nothing is down); D-127's ENDPOINT leg does not reopen. **What survived the re-search and is not in question**: `asia-south1` is in Vertex's own ML-processing-location table alongside Tokyo, Singapore, Sydney and Seoul, so the region's residency property — the leg D-127 actually rests on — stands. |
