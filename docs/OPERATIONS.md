...
| 7 **H** *(was S — raised by D-261)* | Post-call data fidelity, and **what unit is the money in** | `completed` (not `call-disconnected`) carries total_cost + cost_breakdown, recording_url, extracted_data. Transcript parses into TranscriptTurn (check `transcript_lines_unparsed == 0`; if their serializer emits `system:` / `assistant_tool_call:` / `tool_response:` lines, D-260 already skips and counts them — a non-zero count here is the signal, not a failure). Time-to-`completed` (~2–3 min claimed) against our 2-min lead SLO. **THE ONE THAT RAISED THIS TO A HARD GATE: is `total_cost` in DOLLARS or CENTS?** Our adapter divides by 100 (`_ASSUMED_MINOR_UNITS_PER_MAJOR`) and **both sourced signals now argue it should not**: their OSS cost function returns a rounded *dollar* float (`analytics_helpers.py`, bolna-ai/bolna@cd2e192) and every published price is quoted in dollars per minute. Nothing was changed on inference — this is the money path (hard rule 7) and a 100x error in either direction is unacceptable. **The test is one observation:** place one call, read `total_cost` from Get Execution, and put it beside the same call's charge on their dashboard. Two orders of magnitude apart, impossible to misread. Also record whether ANY currency field appears in the payload — `currency_stated=False` on every row today. **A wrong value here means every `usage_event` under-values calls 100x, no spend cap ever arms, and every margin panel reads ~₹0.00.** Evidence: `docs/vendor/bolna/`. |
...
| 16 H | **Does the agent object honour `provider` or `family` — and what selects a non-OpenAI LLM?** [NEW, D-260] | **The BYOK LLM leg may not be BYOK at all.** Read at source in bolna-ai/bolna@cd2e192: `family` — the field our adapter sends — is declared on their `Llm` model and **read by nothing**; the LLM client is chosen by `llm_config["provider"]` against `SUPPORTED_LLM_PROVIDERS` (`bolna/providers.py`), and `Llm.provider` defaults to `"openai"`. Their `LLMProvider` enum has **no `sarvam` member**, so D-36's Sarvam 105B has no obvious value and nothing was guessed — a wrong `provider` is a 400 or a call quietly routed to OpenAI with a Sarvam model name. **The test:** create an agent via `POST /v2/agent` exactly as our adapter does, then `GET /v2/agent/{id}` and read the `llm_agent` block back. Record (a) whether `family` survives, (b) whether `provider` appears and with what value, (c) whether `max_tokens`/`temperature` came back as their defaults `100`/`0.1` — a 100-token cap on every agent reply is a product fact we are currently accepting silently. Then place one call and confirm from the transcript/latency which model actually answered. **Pass** = we can name the field and the value that select Sarvam. **If `provider` is the field**, `ModelConfig` grows an `llm_provider` and `_agent_body` sends it — that is the follow-on work this gate authorises. Blocked outside this repo on: a Bolna account. Evidence: `docs/vendor/bolna/oss-harvest.md` §1. |
