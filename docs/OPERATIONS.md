...
> Azure OpenAI, default `AZURE_OPENAI_DEFAULT_MODEL` — and the residency claim that used to
> be provable from a Vertex URL is now a property of the Azure RESOURCE, which no endpoint
> reveals. Gates 20/20b/20c are what turn that configuration into a verified fact; gate 20c
> is the one a reader underestimates, because Azure's DEFAULT deployment type processes
> worldwide. **The 14-series is retired** with Gemini, and BRD R-04's 16 Oct 2026 date with
> it.
> **THE REGION IS NO LONGER INDIA [D-449, 22 Aug 2026]. `AZURE_LOCATION` is `eastus2` and
> the declared posture is `us-azure-openai`.** Two grounds, and the first is the engine's:
> Bolna's orchestrator is US-hosted — *"By default, Bolna processes calls on infrastructure
> in the US (AWS us-east-1)"* (VERIFIED-VENDOR-DOCS,
> `bolna-findings/mirror/pages/concepts/security.md:29`) — so every in-call turn was a
> us-east-1 → southindia → us-east-1 round trip inside a 350 ms TTFT budget **nobody has
> ever measured** (gate 4). Second, Microsoft's Standard (regional) matrix does not offer
> our shipped default model in `southindia` at all, which is what gate 20b found. **NOTHING
> IN THE 20-SERIES RETIRES.** The posture still pins ONE region and is still Regional
> Standard, so gates 20, 20b, 20c and 20d all survive RE-AIMED at East US 2, and 20d matters
> MORE than it did: the region is now the only residency-shaped statement this product makes
> at all. What DID change is what the claim is worth to a client — the India warranty is
> WITHDRAWN, not narrowed (D-449, SECURITY-COMPLIANCE §4), gate 36's unreachable vendor
> erasure is unimproved by the move, and gate 37(a) now governs the LIVE CONVERSATION rather
> than only the stored recording, because the caller's words leave India as they are spoken.
> **BUT "NO VENDOR DEADLINE IS RUNNING" WAS NEVER A PROPERTY OF THE PRODUCT, ONLY OF
> GEMINI'S CALENDAR** [D-432/W3, 22 Aug 2026]. Azure's models carry retirement dates too;
> until this row was written nothing in the repository knew any of them, so the honest
> statement was "no deadline we have looked for". They have now been looked for, in
> Microsoft's own docs repository rather than a tracker: both allow-listed models retire
> **2027-04-14**, nothing is inside the 120-day lead, and `AZURE_OPENAI_DEFAULT_MODEL`'s
> date was **2026-10-01 until Microsoft moved it eight months in one unannounced edit**
> (commit `138e0f109f`, 2026-07-23) — which is why the dates now live in
> `calevate_shared/model_lifecycle.py` with a source and a read-date each, and why
> `make guardrails` fails on a date that has passed instead of a human rereading a table.
> The reported "2026-03-31 for Standard deployments" was a conflation with **gpt-4o**
> (2024-05-13) in the **Azure Government** doc set: different model, different cloud.
...
| 20 H | **Is the Azure OpenAI resource ACTUALLY in East US 2? — the endpoint cannot tell you, and this is the residency weakening D-410 records rather than papers over** [NEW, D-410; **RE-AIMED, NOT RETIRED, by D-449, 22 Aug 2026**] | **`<resource>.openai.azure.com` names NO region.** Vertex put `asia-south1` in the hostname and in the `locations/` path, so `scripts/check_model_residency.py` could prove residency from the AST; Azure's custom-subdomain form cannot, because the region is a property of the RESOURCE and not of the URL. `AZURE_LOCATION` (`eastus2`) is therefore an ASSERTION that one human confirms once in the Azure portal, and the code says so where a reader will look. **THE MOVE OFF SOUTH INDIA DOES NOT SOFTEN THIS ROW — IT IS THE REASON THE ROW IS NOW LOAD-BEARING.** Under D-410 a wrong region was a broken India promise on top of a working call; under D-449 the pinned region is the ONLY residency-shaped statement the product still makes, so an unread Location field means the posture says nothing at all. **The test:** open the Azure portal, find the resource named in `azure_openai_resource`, and read its **Location** field on the Overview blade; confirm the same value via `az cognitiveservices account show --name <resource> --query location`. **Pass** = both read East US 2, and the reading is filed in `docs/evidence/` with the date and who read it, exactly as the restore drills are. **Wrong answers**: inferring the region from the hostname (it carries none), inferring it from latency, creating a second resource in another region "for failover" — a second region is a second residency posture and needs its own decision-log entry, not a checkbox — and treating "it is in the US anyway" as licence to stop caring which US region it is, which is the same reflex gate 20c refuses. **AND NAME THE DEPLOYMENT AFTER THE MODEL IT SERVES** (`prod-gpt-4o-mini`, not `prod-voice-01`) — VERIFIED-VENDOR-DOCS, `bolna-findings/mirror/pages/providers/llm-model/azure-openai.md`: Bolna resolves the deployment NAME back to a model to pick its handling, and *"a name it cannot resolve is treated as a non-GPT-5 model and gets the wrong defaults"*. Free today, because that IS the correct handling for both members of `AzureOpenAIModel`, and a silent misconfiguration the day a GPT-5 model is adopted (D-417). **What the guard still proves and is NOT a substitute for this gate**: `AZURE_LOCATION` is the only spelling of the region in shipped code, no `Settings` field can carry a region, no Azure endpoint is constructible except through `azure_openai_base_url()`, and that builder cannot emit a region other than the declared posture's. Blocked outside this repo on: an Azure subscription. |
| 20b H | **Which of our two allow-listed models can this subscription actually DEPLOY in East US 2 as Regional Standard — and how much quota does it have?** [NEW, D-410; **premise REVERSED 22 Aug 2026 from Microsoft's own docs repository — D-432/W3; the AVAILABILITY half then RESOLVED BY THE REGION MOVE, D-449, and the QUOTA half survives untouched**] | **THREE FACTS, AND THIS GATE USED TO CONFLATE THE FIRST TWO.** (1) Whether the vendor offers a model in a region, ON A GIVEN DEPLOYMENT TYPE. (2) Whether OUR subscription has quota for it there. (3) When it retires. Only (2) lets a call succeed; only (1) makes (2) possible; and (3) is now a build gate (`make guardrails` → `scripts/check_model_lifecycle.py`). **WHAT CHANGED TWICE, AND THE SECOND CHANGE CLOSED THE FIRST.** This row used to open *"`gpt-4o-mini` is documented available in South India"*, and D-410 chose it as the shipped default over `gpt-4.1-mini` on exactly that asymmetry. Microsoft's own **Standard (regional)** availability matrix says the opposite for `southindia`: `gpt-4.1-mini` is listed, **`gpt-4o-mini` is not** — `gpt-4o-mini` appears in that region only on the **Global Standard** matrix, which is the deployment type gate 20c exists to FORBID. **D-449 DISSOLVES THAT CONTRADICTION BY MOVING THE REGION RATHER THAN THE MODEL.** On the same matrix at the same commit, row `eastus2` (`:23`) carries a ✅ in every one of the first nine model columns, `gpt-4o-mini` (col 9) and `gpt-4.1-mini` (col 4) included, where row `southindia` (`:34`) carries `-` in col 9. So the shipped default does not move, `AZURE_OPENAI_DEFAULT_MODEL` stays `gpt-4o-mini`, and TRD §10's whole cost model is unchanged — the contradiction was `southindia`-only and it went with the region. **WHAT THAT DOES NOT CLOSE, AND THE ROW STAYS H FOR IT:** availability in a region has never been the same fact as quota in OUR subscription, and the retirement dates are a build gate regardless of where the resource lives. This row is RE-AIMED, not answered. Evidence class is unusually strong and is stated so it can be re-checked: Microsoft publishes learn.microsoft.com from the public repository `MicrosoftDocs/azure-ai-docs`, which this environment's git proxy serves even though the doc HOST is egress-blocked (403 on CONNECT, re-measured 22 Aug 2026) — `articles/foundry/openai/includes/model-matrix/standard-models.md:34` at commit `19bbfea4b8` (2026-08-21) versus `.../standard-global.md:35`. ⚠ **It is still carried UNVERIFIED as a statement about today**: the regional matrix's own `ms.date` is 08/12/2025 and its content has not moved in a year, while the global one is dated 07/23/2026. That staleness is precisely why this is a portal gate and not a code change. **THE TEST, and it is now ONE reading that settles three questions.** Portal → Azure OpenAI → **Quotas**, region East US 2: read TPM/RPM for each model family. Then the machine-readable half, which is better than the blade and is the single action that closes this row: the **Models API** (`GET /subscriptions/…/providers/Microsoft.CognitiveServices/accounts/{resource}/models`, or `az cognitiveservices account list-models --name <resource> -g <rg>`) returns, per model and **per SKU**, `lifecycleStatus`, `deprecation` and a `deprecationDate` — so it answers (1) which SKUs are offered to US, (2) alongside the quota page, and (3) the authoritative retirement date, in one call. Then deploy and make one chat-completions call through `azure_openai_base_url(resource)` with `Authorization: Bearer <key>`. **Pass** = a 200, a TPM figure recorded beside gate 13's concurrency numbers, and **the reading filed as `docs/evidence/azure-deployment-attestation.json`** — `{resource_location, deployment_model, deployment_type, read_on, read_by, deprecation_date}`, JSON rather than prose because `check_model_lifecycle.py` CONSUMES it and a guard that parsed a paragraph would fail on a rewording. That file OUTRANKS the dates compiled in `calevate_shared/model_lifecycle.py`; a disagreement is a build failure that says THE PORTAL WINS. Keep the prose account in `docs/evidence/` beside it, as the restore drills do. **Fail** = a 429 with no headroom, or `gpt-4o-mini` absent from the region's Regional-Standard list — which the matrix says it is not, but a matrix whose own `ms.date` is 08/12/2025 is a doc and not our subscription. **THE REMEDY IS NO LONGER HYPOTHETICAL AND NO LONGER SECOND-CHOICE:** switch `azure_openai_model` to **`gpt-4.1-mini`**, which is a live console value (`applies: live`), is priced (`AZURE_LIST_PRICE_USD_PER_MTOK`), has a TRD §10.1 cost row, and retires the same day (2027-04-14). It costs **2.7x** the default on both legs — that is the real bill of this finding and it lands in `billing/rates.py` automatically, not in a code change. A quota increase request is the other remedy and is on Microsoft's clock. **Wrong answers**: deploying into whatever region does have quota — D-449 moved the region ONCE, deliberately, in a decision row and a `Final` constant, and a quota shortfall answered by a second region is a second residency posture arriving without one (D-444); reading the Deployments blade and inferring availability from what is already deployed; and taking the matrix reading above as settled — it is a doc, not our subscription, which is the whole reason this row exists. Blocked outside this repo on: an Azure subscription. |
| 20c H | **Is the deployment REGIONAL STANDARD and not GLOBAL? — Global is the DEFAULT, Global processes worldwide, and no hostname will ever tell you which one you picked** [NEW, D-410; **substance unchanged by D-449, but what the premium BUYS changed**] | **This is the residency trap of the whole migration, and it is one dropdown.** Azure's default deployment type is **Global Standard**, which routes requests to capacity anywhere in the world; **Regional Standard** is what keeps processing in the resource's region. The two are indistinguishable from the endpoint, from the SDK, from the response body and from anything `check_model_residency.py` can read — a Global deployment inside the resource passes every automated check in this tree. **It also costs money, deliberately**: Regional runs roughly 5–10% above Global Standard, with published examples up to +12% and +20%. **WHAT THAT PREMIUM BUYS IS NOT WHAT IT BOUGHT YESTERDAY, AND THE OLD SENTENCE IS NOT REUSABLE.** Until D-449 it bought the India warranty in the client DPA. That warranty is withdrawn. What it buys now is (i) an accurate sub-processor disclosure — we tell clients which region processes their callers' words, and Global makes that sentence unwriteable rather than merely inconvenient — and (ii) the ONLY enforceable property this posture has left, since with the India claim gone the pinned region is the whole of it. **REFUSE THE "IT IS IN THE US ANYWAY, WHO CARES ABOUT GLOBAL" REFLEX; it is the most likely way this gate gets quietly failed.** Global Standard does not mean "somewhere in the US" — it routes to capacity ANYWHERE IN THE WORLD, so accepting it does not downgrade the promise from India to America, it deletes the promise entirely and leaves a DPA naming a region nothing enforces. **D-449 ALSO REMOVED THIS ROW'S SECOND HALF, which is worth stating because it was the expensive one**: gate 20b's finding was that `gpt-4o-mini` is offered in `southindia` on Global Standard and NOT on Regional Standard, so the dropdown that bought residency was the dropdown that took the default model away. In `eastus2` both allow-listed models are on the Regional-Standard matrix, so the trade is gone and nothing here competes with the model choice. Read 20b and this row together, in one portal session. **The test:** portal → the Azure OpenAI resource → Deployments → the deployment named in `azure_openai_deployment` → read **Deployment type**; or `az cognitiveservices account deployment show --name <resource> --deployment-name <deployment> --query "sku.name"`. **Pass** = a Standard (regional) SKU, not a Global one, recorded with the date **in `docs/evidence/azure-deployment-attestation.json` as `"deployment_type": "standard-regional"`** — the same file gate 20b writes, in the vocabulary `calevate_shared/model_lifecycle.py` defines (`standard-regional` · `global-standard` · `data-zone-standard` · `provisioned`, spelled as slugs precisely because the portal's word for the regional one is the bare, misreadable *"Standard"*). `make guardrails` FAILS on any other value, so this reading is enforced from the moment it is filed rather than only when somebody rereads this row. **Record the deployment NAME here too, and make it name the model** (`prod-gpt-4o-mini`): Bolna resolves that string back to a model to choose its handling, and an unresolvable name silently gets non-GPT-5 defaults (D-417, `providers/llm-model/azure-openai.md`). **Wrong answers**: Global Standard, Global Batch, DataZone (a DataZone deployment stays in a geography, not a region, and a geography is not what the DPA names), "we will fix it when we get residency questions" — the client DPA names the region from the day this ships — and **switching to Global to keep a model**, which after D-449 buys nothing at all: both allow-listed models are Regional-Standard in `eastus2`, so a Global deployment now trades the last enforceable property of the posture for convenience. Blocked outside this repo on: an Azure subscription. |
| 20d L | **Does the v1 surface answer on the REGIONAL hostname `eastus2.api.cognitive.microsoft.com`? — because if it does, the AST proof comes back** [NEW, D-410; **RE-AIMED and PROMOTED in importance by D-449**] | **Not a blocker; the only gate here that could make the residency posture STRONGER than it is — and since D-449 the region claim is the ONLY residency-shaped statement left, so the thing this gate could harden is now the whole posture rather than one of its supports.** Azure supports a regional endpoint form alongside the custom subdomain and documents the two as interchangeable, but the OpenAI-compatible **v1** surface is documented only on the custom-subdomain form, and custom subdomains are what Entra ID requires — so D-410 ships the subdomain form and records the regional one as rejected-for-now rather than rejected. The prize is concrete: a hostname carrying the region restores exactly what Vertex had, a residency claim `scripts/check_model_residency.py` can prove from a string literal instead of one a human vouches for at gate 20. **The test:** against the same resource and key, `GET`/`POST` the same v1 path on `https://eastus2.api.cognitive.microsoft.com/openai/v1/...` and compare the response with the custom-subdomain call. **Pass** = identical behaviour, in which case `azure_openai_base_url()` moves to the regional form, the guard gains a literal to check, and it lands as its own decision-log entry naming this gate as the evidence. **Fail** = anything other than identical (a 404, an auth difference, a required `api-version`), in which case the current shape stands and this row records why. **Wrong answer**: shipping the regional hostname on the strength of the docs' "interchangeable" without making the call — that is the premise class D-31/D-32 exist for. Blocked outside this repo on: an Azure subscription. |
...
| 35 H | **What does `POST /call/{execution_id}/stop` actually ANSWER, in each of the four states it can be sent in?** [NEW, 21 Aug 2026, D-432] | With four executions in hand, send a stop to each and record the status and body: **(a)** one genuinely queued and not yet ringing; **(b)** one ringing but unanswered; **(c)** one already connected — the vendor says the route *"cannot stop a call already in progress"* (`bolna-findings/mirror/pages/build-with-ai/skills/make-call/SKILL.md`) but does not say what it RETURNS when you try; **(d)** one this platform has already stopped. Also record whether a stopped execution later appears in `GET /executions` and with which `status`. | **`dial_recall_unstopped`'s count is only meaningful once (b), (c) and (d) are distinguishable.** The recall job counts any refusal as "could not stop", which is the safe direction — it over-reports rather than under-reports the phones that will still ring — but an operator reading that alarm mid-incident needs to know whether the number means "already ringing" or "the vendor does not know this id". If (d) is a documented refusal code distinct from (b), the job can stop counting it at all. If a stopped execution reconciles to a status other than `failed`/`cancelled`, `_STATUS_MAP` needs a member and the campaign contact's retry ladder needs a rule — a dial WE cancelled must not consume the contact's attempts. Needs a vendor account: this is four real calls, not a doc question, and the mirror is silent on all four. |
| 36 H | **The vendor DPA deletion clause — the erasure obligation no code in this repository can discharge** [NEW, 22 Aug 2026, D-433; **more important after D-449, and unimproved by it**] | **THIS GATE IS NOT AN EXPERIMENT AND HAS NO PASS CONDITION A TEST CAN CHECK. It closes with a SIGNATURE.** **D-449 RAISED THE PRICE OF THIS GATE WITHOUT MOVING IT ONE INCH.** Moving the language model to `eastus2` changes nothing about vendor erasure reach: `docs/evidence/subprocessor-erasure-reach.md` §1 and §4 stand exactly as recorded — no vendor DELETE route reaches a subject's recording or transcript, and **no retention category in this repository issues any vendor-side call at all**. What changed is that the contractual leg now carries more of the load, because the argument "at least the inference stayed in India" is no longer available to sit beside it. The engineering question is already answered and answered negatively: `docs/evidence/subprocessor-erasure-reach.md` §1 enumerates every `DELETE` route Bolna documents across all 335 mirrored pages — ten routes, nine of which delete configuration objects, and the Executions surface is four GETs (`api-reference/executions/overview.md:11-16`) while the Calling surface is one POST (`api-reference/calls/overview.md:11-13`). **No route deletes an execution, a recording or a transcript at subject granularity**, and their stated retention for those copies is *"contact support for retention policy"* (`concepts/security.md:19`) — i.e. unbounded. So a DPDP §12 erasure completes in our systems and leaves a recording and a transcript of the caller at a US sub-processor, indefinitely, with the DPDP §8(7) storage-limitation exposure sitting on us as Data Fiduciary. **What must be in the signed DPA**, wording in `docs/evidence/subprocessor-erasure-reach.md` §6 ready to paste: (a) **Deletion on Instruction** — deletion of a named subject's recordings, transcripts and derived fields within 30 days of written instruction, 90 for backups, confirmed in writing; (b) **Subject-Granular Capability** — the operative clause, because without it the only deletion on offer also destroys the client's live agent and every other caller's records; (c) **Stated Retention** — an actual number, since today there is none; (d) **Deletion on Termination**; (e) **Sub-processor Flow-Down**. **Ask gate 9's item (iv) and gate 12(f) in the same conversation** — they are the same negotiation and gate 12(f) ("recording retention + DPDP deletion API") is now half-answered: there is no API, so all that is left of it is the contract. **Equivalents are owed for Sarvam and Microsoft.** ⚠ **SARVAM'S IS NO LONGER UNKNOWN — this cell said it was, and it was answered on 27 Aug 2026 by the founder reading the published Privacy Policy** (`sarvam.ai` is still egress-blocked here; the reading was relayed, not fetched). Its retention table: Content (Inputs/Outputs) **30 days after last access by default, described as user-configurable** — ⚠ nobody has located WHERE that setting is changed, so do not record it as configured; Account & Profile data **account + 90 days** unless earlier deletion is requested in writing; Voice Samples & Models **until consent withdrawal + 30 days**; Security Incident Logs **7 years**. Deletion: *"within 30 days of request verification"*, except where retention is required by law, where data is needed for ongoing legal proceedings, or where technical limitations prevent it (*"we will anonymize instead"*). **That is a published policy, not a negotiated term** — the DPA equivalents (a) to (e) above are still owed, and gate 40 is where the Sarvam side of that conversation is held. Azure's REPORTED 30-day abuse-monitoring copy is better removed by applying for modified abuse monitoring than by a deletion term. **What is ours and is DONE, so that this gate is not mistaken for the whole problem** (D-433): the erasure certificate no longer implies the vendor copies are gone, every erasure opens a `processor_erasure_tasks` row naming the vendor ids to quote, `processor_erasure_overdue` pages at 30 days, and `runbooks/processor-erasure.md` is the manual procedure. **A `refused` answer on one of those tasks is the evidence that makes this gate urgent** — route it to whoever is negotiating. Blocked outside this repo on: a signed commercial term with each vendor. Nothing in this repository can close it, and any change that appears to has narrowed a claim it was not entitled to narrow. |
| 37 H | **The two questions for the advocate that decide whether we may run a call at all — and both are YES/NO, deliberately** [NEW, 22 Aug 2026; ours, not Bolna's — same class as gates 15 and 19] | **NOT A PILOT ITEM AND NOT A PAPERWORK ITEM.** `PENDING_LEGAL_REVIEW = true` (`apps/web/src/lib/legal/placeholders.ts`) puts a "do not rely on this page" banner across all eight legal documents, and it comes off when an Indian advocate has reviewed them. These are the two questions that review must answer, written as yes/no because a paragraph of hedging from counsel is the same as no answer. **(a) Is the recording of an ordinary business telephone call "biometric information" within rule 3 of the IT (SPDI) Rules 2011, whose definition of biometrics includes VOICE PATTERNS?** If YES, call audio is sensitive personal data until 13 May 2027, the SPDI transfer test (comparable protection at the destination plus consent or contractual necessity) binds every leg — including the voice platform's own copy of the recording in the **United States** (gate 9, D-415) — and the client DPA's consent architecture changes. **D-449 MADE (a) MORE CONSEQUENTIAL AND CHANGED WHAT IT GOVERNS.** Until D-449 the leg most obviously in scope was the STORED recording at the US orchestrator; the language model sat in India and the live conversation could be argued to stay there. It does not any more: the in-call LLM leg is `eastus2`, so the caller's words cross the border **as they are spoken**, on every turn, whether or not any recording is ever kept. A YES therefore reaches the live call and not only the archive — and SPDI rule 7's comparable-protection leg is UNEVIDENCED today, because zero sub-processor DPAs are signed (gate 36). If NO, the position is what `/legal/dpa` clause 9 already describes. **We have deliberately not answered it either way**; the clause says the question is undecided and says who is answering it, which is the only honest thing a document under a pending-review banner can do. **(b) Does the phased commencement of the DPDP Act — sections 3–17 from 13 May 2027 — leave any gap in our lawful basis for cross-border transfer between today and that date, given that §16(2) preserves the SPDI regime meanwhile?** Ask it in that shape: the tempting answer ("§16 permits it, nothing is notified") is about a section that is not in force. **Also settle, in the same sitting, three facts this environment could not verify at all**: the exact notification date of the DPDP Rules 2025 (this tree carries 14 Nov 2025, a second synthesis says 13 Nov — SEC-COMP §4); whether MeitY's January 2026 consultation on pulling the cross-border provisions forward and compressing the SDF deadline has since been notified; and whether any SDF class notification could reach a voice-AI processor, which is the only thing that would arm Rule 13(4)'s localisation power over us. **WHY THIS IS AN H.** Everything in the 20-series is a fact about a console that a person can read in a minute. This is the one gate whose answer can change what the product is allowed to do with a caller's voice, it is live for the whole window in which client #1 goes live, and **every legal source in this class is egress-blocked from this environment** (`indiacode.nic.in`, `meity.gov.in`, `egazette.gov.in`, `irdai.gov.in` — re-measured 22 Aug 2026), so no lane can close it by reading. Blocked on ONE external thing: an Indian advocate. File the answers in `docs/evidence/` with a date and a name, the way gates 20 and 20c are filed. |
| 38 H | **Which REGION is the Sentry organisation in — because it is chosen once, at ORGANISATION creation, and there is no India** [NEW, 22 Aug 2026, D-451; ours, not Bolna's — same class as gates 15, 19 and 37] | **NOT AN EXPERIMENT. It closes with a screenshot of the organisation's settings and one line in the decision log, taken BEFORE the organisation exists if it does not yet.** Sentry runs two data regions and only two — US (Iowa) and EU (Frankfurt). The choice is made when the ORGANISATION is created, it is baked into the DSN host, and it is permanent: moving means a NEW organisation, a NEW DSN, and the loss of every issue, alert rule and integration behind it. Nothing in this repository can detect it or constrain it — `apps/api/core/observability.py` reads `settings.sentry_dsn` and nothing else, `_dsn_problems` validates the DSN's SHAPE and never its host, and no `Settings` field names a Sentry region (deliberately: a `sentry_region` field would be console-editable via `managed_fields()`, read by nothing, and would dress an irreversible vendor decision as a config row — the D-95 §4 failure). So the gate is the only place this can be owned. **PASS = (a) the region is recorded, by name, in the decision log entry for D-451; (b) the DSN host in the deployment's `SENTRY_DSN` matches the region recorded — `o<org>.ingest.sentry.io` for US, `o<org>.ingest.de.sentry.io` for EU — checked by eye, since no test can; (c) if no organisation exists yet, the choice is made deliberately at creation rather than by accepting the default.** **WHAT IS AND IS NOT AT STAKE, because conflating them is the likely error: this decides where our STACK TRACES live, not where a transcript goes.** Hard rule 6 keeps personal data out of Sentry at the source and that has not moved — `apps/api/core/observability.py:1412-1413` (`send_default_pii=False`, `max_request_body_size="never"`), `:1443` (`include_local_variables=False`), `:1468-1469` (`before_send=scrub_event`, `before_breadcrumb=scrub_breadcrumb`), and no `traces_sample_rate` at all (`:1470-1474`), each pinned by a mutation test in `tests/observability_readiness_guard_test.py`. The sub-processor register already tells clients error monitoring is operated from outside India, which is true of BOTH regions, so the CONTENT of the answer changes no client-facing sentence either way. That it is unowned, permanent and invisible to every automated check is the whole reason it has a row. **A red here does not reopen the engine decision** — it is H because it cannot be revisited later, not because it blocks a call. |
| 39 H | **Does `GET https://api.frankfurter.dev/v2/rate/USD/INR?providers=FBIL` actually return the four fields the adapter parses — and does the FBIL filter actually answer?** [NEW, 27 Aug 2026; ours, not Bolna's — same class as gates 15, 19, 37 and 38] | **THE ONE COMMAND, run from a box with egress:** `curl -sS 'https://api.frankfurter.dev/v2/rate/USD/INR?providers=FBIL' -D -` and paste the headers AND the body. **PASS = (a)** HTTP 200 with a JSON object carrying exactly `date`, `base`, `quote`, `rate`, with `base` = `USD` and `quote` = `INR`; **(b)** `rate` is a JSON number in a plausible band (roughly 80-95 at the time of writing) and NOT a string; **(c)** `date` is an ISO date no later than today and within a few business days of it — that is the FBIL publication date, and it is what `core/fx.MAX_QUOTE_AGE` is measured against; **(d)** the `providers=FBIL` filter is honoured rather than ignored — compare the number against the same request with `providers=ECB` and confirm they differ, because a silently-ignored filter means the platform is converting at a fifty-central-bank blend nobody can look up; **(e)** record any `RateLimit-*`/`Retry-After` headers and the `cache-control`, since the rate limit is the one fact about this vendor that NO primary source read this session states — it is UNKNOWN and is recorded as UNKNOWN in `apps/workers/fx_pull.py`. **WHY IT IS A GATE AND NOT A TEST.** Every foreign-exchange API host is egress-blocked from this environment (403 on CONNECT, measured 27 Aug 2026: `api.frankfurter.dev`, `api.frankfurter.app`, `open.er-api.com`, `api.exchangerate.host`, `api.fxratesapi.com`, and `www.ecb.europa.eu`/`www.rbi.org.in` directly), so **no byte of a live response has been seen**. The adapter is written against the vendor's OWN OpenAPI document and Ruby source, read at pinned commit `60541ad190b6e192d2969038b2b299fb4800a8a2` of `github.com/lineofflight/frankfurter` — `lib/public/v2/openapi.json` for the `Rate` schema and the `providers` parameter, `lib/provider/adapters/fbil.rb` for the Indian publisher, `lib/versions/v2.rb::cache_control_for` for the daily cadence — which is VERIFIED-VENDOR-DOCS for the CONTRACT and evidence of nothing about the live endpoint. **WHAT A RED COSTS, and why it is survivable:** nothing silently. A response this parser does not recognise is REFUSED with the field named, the pull tick fails its ladder and alerts `fx_pull_failed`, and every vendor cost keeps converting at the operator's configured `USD_INR_RATE` exactly as it did before this feature existed. So a red here is a feature that does not start, never a wrong number that does. **ALSO SETTLE IN THE SAME SITTING, because it is a commercial term and not an experiment:** the hosted endpoint's terms of use for COMMERCIAL traffic. The software is MIT (`LICENSE`, "Copyright (c) Hakan Ensari") and that covers the SOFTWARE, not the hosted service or the underlying central-bank data, each of which carries its own `terms_url` in the `/providers` response. If the terms are unsuitable, the escape hatch is already built and costs one constant: the same software self-hosts from a documented one-command Docker image, and `RATE_URL` moves to our instance with nothing else in the tree changing. |
| 40 H | **The Sarvam commercial terms — the ONLY instrument that can displace their training clause, plus the retention control nobody has found** [NEW, 27 Aug 2026, ours, not Bolna's — same class as gate 36: it closes with a SIGNATURE, not a measurement] | **NOT AN EXPERIMENT AND NOT AN ENGINEERING TASK.** Sarvam **ToS v2.0, effective 29 July 2026, s.17.5** says: *"Where and to the extent we use Inputs, Outputs or usage data for training our machine-learning models, we will do so in accordance with the Privacy Policy and applicable law, and (where required) subject to your consent, which you may decline or withdraw in accordance with the Privacy Policy; in such cases, access to certain Offerings may be restricted."* Two things follow and neither is negotiable from inside this repository. **(a) The clause does NOT vary by tier** — free credits, Starter pay-as-you-go and paid plans are treated identically — so upgrading the plan buys nothing here. **(b) s.6.2 gives a signed order form or enterprise agreement precedence over the Terms**, which makes it the only route to a no-training commitment on the speech leg. **CLOSES WITH:** (i) a signed order form or enterprise agreement carrying a no-training term, a stated retention period and a subject-granular deletion commitment (take it into the same conversation as gate 36's (a)-(e) — it is one negotiation, not two); OR (ii) a recorded decision to run on the default terms, in which case the client-facing text stays as it is, because it already says exactly that. **Until one of those, the disclosure IS the answer**: `/legal/dpa` cl.2, `/legal/privacy` §6, `/legal/terms` §§7-8 and `/legal/subprocessors` §3.4 state the vendor position rather than promising it away, and no document may re-widen the sentence. **A SECOND, SMALLER ITEM ON THE SAME ACCOUNT, and it is a console reading rather than a signature:** the Privacy Policy calls the 30-day Content (Inputs/Outputs) retention *"user-configurable"* and **nobody has located where it is configured** (the founder looked on 27 Aug 2026). Find the control, record what it is set to, or record that it does not exist in the dashboard — and until then no document in this tree may say we have shortened it. Blocked outside this repo on: a Sarvam account holder at the console, and a signed commercial term. Evidence: VENDOR-PUBLISHED, Sarvam ToS v2.0 ss.6.2/17.5 and Sarvam Privacy Policy, read by the founder at `www.sarvam.ai` on 27 Aug 2026 and relayed — ⚠ `sarvam.ai` and `docs.sarvam.ai` are egress-blocked from this container, so nothing here re-verifies it by fetching. |
...
moved it again, to Azure OpenAI (`eastus2` since D-449), so this no longer decides the leg — it
