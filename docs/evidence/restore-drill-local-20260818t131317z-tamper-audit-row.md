# Local restore drill — 20260818t131317z
...
- Sabotage mode: tamper-audit-row
- **Verdict: RED**
- Wall clock: 12.7s
...
|---|---|---:|---|
| `preflight` | ok | 0.37 | postgres 16.13 (Ubuntu 16.13-0ubuntu0.24.04.1), age 1.1.1, s3 at http://127.0.0.1:9000 |
| `provision-source` | ok | 3.55 | calevate_drill_src_20260818t131317z at head f1c8b7d5a903, 14 leads across 2 tenants, 6 audit entries |
| `dump` | ok | 0.15 | 269967 bytes with production flags (--format=custom --compress=6 --file --dbname) |
| `encrypt` | ok | 0.02 | age --encrypt --recipients-file --output -> 270231 bytes, sha256 80d810bad38587bb…; plaintext removed |
| `upload` | ok | 0.06 | s3://calevate-drill-20260818t131317z/postgres/2026/08/18/calevate-20260818t131317z.dump.age + evidence json |
...
| `fetch` | ok | 0.03 | downloaded 270231 bytes, sha256 matches evidence |
| `decrypt` | ok | 0.01 | 269967 bytes recovered |
| `restore` | ok | 0.73 | restored into calevate_drill_restore_20260818t131317z |
| `verify` | **FAIL** | 7.43 | 1 of 8 FAILED: audit_chain |
| `cleanup` | ok | 0.29 | dropped 2 scratch db(s), deleted 4 objects, removed the decrypted working copy |
...
| `audit_chain` | **FAIL** | 6 entries recomputed, complete=True, breaks=1 ['01a01501-48b7-7261-b294-bc462b834533:content'] |
...
- database `calevate_drill_src_20260818t131317z`
- database `calevate_drill_restore_20260818t131317z`
- object-store bucket `calevate-drill-20260818t131317z`
