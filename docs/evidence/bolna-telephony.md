...
| `api-reference/phone-numbers/get_all.md` | **Fact captured**: `renewal_at`, `price` as a `$`-prefixed string, `rented` boolean. Id typed `^[0-9a-fA-F]{32}$` — see gate 25. |
...
- **`NumberPurchaseIn.series` / `.city` (`provisioning_routes.py:64-65`).** The route's
  ...
  needs the account (gate 26), not an edit today. Recorded here so the day it is edited, the
  reason is on file.
