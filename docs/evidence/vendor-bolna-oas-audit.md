...
| Claim | Spec element |
|---|---|
| Costs are **cents** (`_ASSUMED_MINOR_UNITS_PER_MAJOR = 100`) | "in cents" on `AgentExecution.total_cost`, `.cost_breakdown`, and all five `CostBreakdown` members. **Retires the D-261 marked assumption in our favour** — the constant never changes, it stops being a guess |
...
|---|---|---|
| Which currency's cents is `total_cost`? | The spec says "cents" and names no currency | `curl -sS -H "Authorization: Bearer $BOLNA_API_KEY" https://api.bolna.ai/executions/$EXECUTION_ID \| jq '{total_cost, cost_breakdown}'` beside the same call's charge on the dashboard |
...
| Is `usage_breakdown` populated in practice? | Declared ≠ emitted | same `GET /executions/{id}`, `jq .usage_breakdown` |
