...
| 14 | `max_tokens` / `temperature` are the vendor's problem | **VERIFIED-OSS** — unsent, so defaults apply: `max_tokens=100`, `temperature=0.1` | **NOT changed — noted in harvest.** A 100-token cap on agent replies is a real quality knob we are silently accepting a default on; it is a product decision, not a bug, and it rides on gate 16's read-back. |
