...
Ten findings across twelve sites, all fixed. Four decision-log entries: **D-220, D-221, D-222, D-223**.
...
**Re-run: both shells report zero.** Decision log **D-222**.
...
sabotage results are now in its header. Decision log **D-223**.
