...
| **The telephone carrier** (Exotel · Vobiz · Plivo — none selected) | the caller's number, the number dialled, the carrier's own call detail records, and **the live audio of the call in both directions**. Since D-592 the conversation runs in a container of ours and the media leg terminates at the carrier: `apps/voice-worker/voice_worker/carrier.py` reads 8 kHz mu-law media frames off the carrier socket both ways. | **UNKNOWN.** No carrier is chosen and no account is open with any of the three. `www.plivo.com` and `api.plivo.com` are EGRESS-BLOCKED from this container (measured 13 Sep 2026, `pre-build-blockers-2026-09-13.md` §10), and no page of Exotel's or Vobiz's has been read here either. | **UNKNOWN.** Nobody has read a carrier retention position. Indian licensees keep call detail records under their own licence conditions — that is a claim about a regulator nobody here has checked against a primary source, so it is NOT asserted. | **UNKNOWN.** The only carrier REST call verified anywhere in this tree is the HANGUP (`pipecat/serializers/plivo.py:184`, PIPECAT SOURCE); `apps/api/engine/pipecat.py` refuses every other carrier REST call by name. No deletion route has been read, quoted or called. | UNKNOWN — and that is the finding. The vendor holding the MOST of a call is the one about which least is established. |
...
**⚠ THE CARRIER ROW IS NEW (18 Sep 2026) AND ITS ABSENCE WAS THE HOLE IN THIS TABLE.**
`compliance/processor_erasure.PROCESSORS` was `("voice_engine", "speech", "llm")`, the DB
CHECK matched, and both erasure paths opened vendor tasks against those three only — while
the published sub-processor register told clients the carrier receives *"Caller and called
numbers, call detail records, and the live audio of the call in both directions"*
(`apps/web/src/lib/legal/subprocessors.ts`). So a §12 certificate enumerated three vendor
copies and was silent about the one holding both the number and the sound of the call.
`telephony` is now the fourth role in that vocabulary and
`deletion.ERASURE_EXCEPTIONS` carries the client-facing statement
(`TELEPHONY_OUTCOME = "not_reached_vendor_unestablished"`). Its own outcome word rather
than the speech/language one is deliberate: `not_reached_no_api` means we looked and there
is no route, and here **nobody has looked** — hard rule 11 forbids borrowing the stronger
finding. ⚠ **NO TASK CAN BE OPENED YET**: the CHECK `processor_is_known` on
`processor_erasure_tasks` still admits the original three (migration `c9f4a2e17b83`), so
widening it is a migration, and until it lands the register carries the obligation.
