...
vendor documentation readable here is the Bolna mirror. Sarvam and Azure rows below are
...
| **Sarvam** (Saaras STT · Bulbul TTS) | raw call audio in, recognised text out; and the first post-call extraction sends it the **raw, un-redacted transcript** (`apps/workers/extraction.py`, `GEMINI_EXTRACTION_DEFAULT is False`) | India (sovereign by vendor) — the residency claim is fine here; **erasure is not residency** | **UNKNOWN.** No retention statement exists anywhere in this tree. `docs.sarvam.ai` is egress-blocked from here. | **UNKNOWN.** No deletion API is known, called, or recorded. | UNKNOWN |
