...
transport.input()
  ...
  → TTS            Gnani timbre-v2.5 on Clear; Cartesia sonic-3.5 on Studio (D-629 —
                       Sarvam left the TTS leg entirely; it is still the STT above)
  → transport.output()
...
**THE RATE LIMIT'S UNIT IS UNKNOWN, AND THE NUMBER IS PUBLISHED AFTER ALL.** ⚠ **THIS
PARAGRAPH READ "THE RATE LIMIT IS UNKNOWN, NOT 60-WITH-AN-AMBIGUOUS-UNIT … there is no
published Gnani rate limit" UNTIL 19 Sep 2026, AND THE NO-PUBLICATION HALF WAS WRONG**
(D-631, the same hard-rule-11 failure as the price). The founder's reading of
`docs.gnani.ai` on 15 Sep 2026 found no rate limit on those four pages, which was a true
statement about the PAGES and was written down as one about the VENDOR. Their own pricing
console states **60 requests/minute** beside the ₹27.00 / 10,000-character rate
(`app.gnani.ai/voice/pricing`, read by the founder 19 Sep 2026 and relayed —
**VENDOR-PUBLISHED**, founder-relayed, not fetched here). **What the section gated on is
STILL open, and it is the half that decides the leg**: the page states the number and not
its UNIT or SCOPE — per key, per account, per model, per connection, or per utterance — so
the original question ("a WebSocket session or each utterance?") has a figure now and still
no answer, and a limit counted per utterance is the one that kills us. It remains a question
for the founder to put to Gnani with the other two (OPERATIONS §2 gate 56), not a research
task for this container: `docs.gnani.ai`, `gnani.ai` and `api.vachana.ai` are all
EGRESS-BLOCKED here (HTTP 000, measured 15 Sep 2026), and `app.gnani.ai` is refused by the same egress
proxy (`curl: (56) CONNECT tunnel failed, response 403`, measured here 19 Sep 2026) — which
is why the pricing reading is the founder's and is labelled as such.
...
| 4 | The worker pipeline, Sarvam TTS, local run against a fake transport. ⚠ **THE SARVAM TTS HALF OF THIS ROW WAS WITHDRAWN BY STEP 9 (D-629, 18 Sep 2026) AND THE ROW IS LEFT AS THE DATED PLAN** (noted 19 Sep 2026): `voice_worker/pipeline.py::_build_tts` has no Sarvam arm, no default provider and no default model, and an agent naming no `tts_provider` RAISES rather than falling through. `_build_stt` is untouched — `saaras:v4`, `SARVAM_API_KEY`, `SarvamSTTService` — so Sarvam is still on this pipeline, on the other leg | step 1 |
...
| 8 | `GnaniTTSService` | **DONE, 15 Sep 2026 (D-618).** Not gated on "Gnani Q1" in the end, because that question had no answer to wait for: the cap it named is not in the current documentation. ⚠ **THAT LAST CLAUSE IS SUPERSEDED BY D-631 (19 Sep 2026): THE CAP IS PUBLISHED AFTER ALL — 60 requests/minute, on Gnani's own pricing console** (`app.gnani.ai/voice/pricing`, founder-read, VENDOR-PUBLISHED), which was true of the four pages read on 15 Sep and not of the vendor. **The row's DECISION still stands**: what the question actually needed was the cap's SCOPE (per key, per account, per model, per connection, per utterance), and the page states the number and not its unit — so it would still have been wrong to gate on it. OPERATIONS §2 gate 56(b) carries what is left. §5 has what was built, what was found by running the vendor's plugin against our pin, and the three unknowns that remain |
| 9 | `Clear` flips to Gnani; Sarvam TTS retires | **THE VENDOR HALF IS DONE (D-629, 18 Sep 2026); THE PRICE HALF IS HALF DONE (19 Sep 2026) AND THE REMAINING HALF IS ONE OPERATOR ACTION.** The flip: `TtsModel` is `Literal["sonic-3.5", "timbre-v2.5"]`, the `sarvam` TTS provider is out of the catalogue, Clear is Gnani's and Studio is Cartesia's, so no Sarvam voice can be published any more. ⚠ **THIS ROW SAID GNANI PUBLISH NO PRICE "no per-character rate, no per-second rate, no currency, no free tier, nothing" AND THAT WAS A NOT-FINDING WRITTEN DOWN AS A FACT** — the hard rule 11 failure. Gnani's own console publishes **₹27.00 / 10,000 characters** and a **60 requests/minute** limit (`app.gnani.ai/voice/pricing`, founder-read 19 Sep 2026). That figure is now `billing/rates.TTS_INR_PER_10K_CHARS` and it re-strikes the Clear rung's COST FLOOR, replacing a withdrawn vendor's ₹30 that had been frozen in its place; TRD §10.1 and §10's summary both state it, and `check_docs_drift` §4b guards TWO rungs again. ⚠ **CLEAR STILL CANNOT BE SOLD, AND THAT IS HARD RULE 7 WORKING RATHER THAN A GAP.** A published catalogue price is not an invoice: `agents/voice_offer.tts_price_is_billable` opens on an operator's attestation, exactly as the Gemini catalogue price is `verified=False` on the LLM leg. So every screen and price page still says the rung is unavailable (`apps/web/src/lib/api/rateCard.ts::UNPRICED_TIER_NOTICE`, rendered by `/pricing`'s rate table, the ROI calculator and the client console's "What calls cost"; the picker renders the server's own per-voice refusal). ⚠ Two further claims in this row were already stale: `voices.VOICE_TIER_OF_PROVIDER["gnani"]` is **not** `None` — it has been `VALUE_VOICE_TIER` since 18 Sep 2026 (`clear` since the 19 Sep rename), and what refuses a Gnani voice is the OFFER seam and never the tier map. **What is left is one attestation** of a real Gnani invoice figure in the ops console, after which a rate can freeze on a credit lot and `check_model_lifecycle` goes RED until somebody reads a Gnani lifecycle page. Everything else for the flip is built |
...
| 17 | The post-call pipeline runs for an `owned_runtime` call — extraction, CRM columns, moments, knowledge gaps, the lead, the hot-lead alert, the CRM fan-out | **DONE, 15 Sep 2026 (D-607).** This row is here because the seam was MISSING and nothing in the table said so: steps 4 and 11 land a worker that writes `calls`, `transcript_turns` and `usage_events`, and then nothing happened. There is no webhook on this engine (§3 category D — nothing external calls us) and no poller (`PipecatEngine.list_executions` reports nothing, on an independence argument that is correct for a poller and says nothing about a call we already know about), so every call this engine handled lost extraction, the CRM columns, the lead and the hot-lead alert — silently, and totally. Closed with the transactional OUTBOX, not a poller and not a new deployable: `voice_worker/sink.py::settle` sends ONE request and `apps/api/worker/service.py::settle_call` writes the call row, the ledger (or its refusal) and ONE `outbox_messages` row in ONE transaction, and `apps/workers/dispatcher.py::dispatch_outbox` — which already exists, already runs every ten seconds and already owns Redis — hands it to `apps/workers/pipeline.py::run_post_call_pipeline`, the SAME function the Bolna path reaches. ⚠ **THIS ROW PUT THOSE WRITES IN `voice_worker/sink.py::settle` AND A READER WOULD GREP IT FOR SQL AND FIND NONE** (corrected 19 Sep 2026): the sink is the CLIENT, and D-621 is why — `MeteredQuantity` carries no money field, the worker sends quantities and the server prices them, so `billing/rates.py` stays the one door a rupee comes through. Idempotent on the partial unique index over `dedupe_key` (`post-call:<calls.id>`), so a re-settlement promises nothing twice. The half that made it possible: `SqlControlPlane.execution()` now reads the call back out of our own rows, finding the tenant by PARSING the engine-space id the sink mints (`calevate_shared.engine.pipecat_call_ref`) — `calls` is FORCE-RLS'd and `get_execution` carries no tenant, and the alternatives were widening a policy or adding a routing table. ⚠ **`_upsert_lead` STILL FILES NO LEAD**, and that is BLOCKER-1 rather than this seam: §1.2 gives the party numbers to the carrier's CDR, the worker is handed none, so `calls.from_e164` is NULL and the lead has no key. Extraction, the moments, the knowledge gaps and the call row's summary/sentiment/outcome all land today; the lead, the hot-lead alert and the CRM fan-out land the day step 6 fills those two columns, with no further edit — `execution()` reads them from the row |
...
- **Gnani's price — RESOLVED 19 Sep 2026, and this bullet was wrong in the confident
  direction.** It read "there is none published at all". Gnani's own console publishes
  **₹27.00 per 10,000 characters** for Text to Speech at `app.gnani.ai/voice/pricing`
  (read by the founder and relayed with a screenshot; the host is not reachable from this
  container). What was true is the *provenance* correction beside it: the ₹27 figure
  already in this tree came from a **RESELLER's** page for THEIR platform
  (callmissed.com), and refusing it was right — a figure that later turns out to match is
  not a source, and re-verifying the reseller would never have made it a Gnani rate. The
  rate now lives at `billing/rates.TTS_INR_PER_10K_CHARS` and prices the Clear rung's cost
  floor. ⚠ **STILL OPEN: no Gnani minute reaches `unit_cost_paid` without an operator
  attestation** — a catalogue price is not an invoice, hard rule 7. §5, §6 step 9, D-618.
- **Gnani's rate limit — the NUMBER is now published and its SCOPE is not.** The same
  pricing page states **60 requests/minute**. ⚠ This bullet previously called the number
  itself unknown, having been burned by an earlier version that presumed it. What remains
  UNKNOWN is whether it counts per key, per account or per model, and whether a request is
  a session or an utterance — which is what decides how many simultaneous calls this rung
  can carry. Nothing here has costed that. §5, gate 56.
...
| Tier | Before D-592 | After D-592 | Today (D-631, 19 Sep 2026) |
|---|---|---|---|
| Clear | ₹4.1211 | ₹3.3111 (0.95 + 0.50 STT + 0.2411 LLM + 1.62 TTS) | **₹3.1491** (0.95 + 0.50 STT + 0.2411 LLM + **1.458** TTS) |
| Studio | ₹5.5899 | **₹4.7099** | **₹4.7099** (unmoved) |

⚠ **THIS TABLE HAD TWO COLUMNS AND ITS "Now" FOR CLEAR WAS ₹3.3111 UNTIL 19 Sep 2026.** The
Clear TTS leg was still ₹1.62 there — 540 chars at a withdrawn vendor's ₹30 / 10,000 — and
D-631 struck the scalar at Gnani's own published **₹27.00 / 10,000 chars**
(`billing/rates.py::TTS_INR_PER_10K_CHARS`), which is ₹0.9720–₹1.4580 per call-minute
over §10.1's assumed 360–540 chars. Read from the code on 19 Sep 2026:
`rates.SELF_SERVE_COST_FLOOR_INR_PER_MIN` = **3.1491** and
`rates.CARTESIA_COST_FLOOR_INR_PER_MIN` = **4.7099**. Neither is typed anywhere — each is
derived from its own legs by a function in `billing/rates.py` — so this table is a
transcription and the code is the authority. ⚠ **A FLOOR IS NOT A LICENCE TO SELL**: no Clear minute may be metered until an
operator attests a Gnani invoice figure (hard rule 7).

Which legs move with the dollar: the **engine** (USD) does. **STT** (Sarvam, in rupees) and
**the Clear TTS leg** (Gnani, in rupees) do not. ⚠ This sentence said "**STT** and **Sarvam
TTS** are published by Sarvam in rupees"; Sarvam left the synthesis leg on 18 Sep 2026
...
* `cost_floor::test_the_approved_card_clears_both_floors_and_every_rung_clears_target`
  ...
  encoding the rule in them. ⚠ **The name above no longer exists in the tree and this
  bullet gave no way to find the test it describes** (noted 19 Sep 2026): it is
  `tests/cost_floor_test.py::test_the_approved_card_clears_both_floors_and_thin_cells_are_warned_not_refused`,
  which carries the rename in its own docstring.
