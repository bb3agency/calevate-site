...
| R-04 | Gemini 2.5 family retirement 16 Oct 2026 | Certain | **Low for the extraction leg — NEUTRALISED. Re-opened as a DATED gate for the dashboard leg (D-142).** | Sarvam 105B is free per token and runs the extraction leg permanently (`GEMINI_EXTRACTION_DEFAULT is False`, D-127 G-7), so no post-call path was ever exposed to this date, and that half is closed. The one Gemini path that exists — the user-triggered dashboard AI on Vertex AI `asia-south1` — was written on **3.x Flash-Lite** (`calevate_shared.engine.GEMINI_DEFAULT_LLM`) rather than migrated to it later, and `tests/sarvam_model_identifier_test.py` fails the build on any shipped module that names a 2.x identifier. **What the 16 Aug 2026 re-search changed**: `GEMINI_MODEL_CONFIRMED_IN_REGION is False` — the 3.x family is reported on the global and `us`/`eu` multi-region endpoints, and what is reported in Mumbai is the 2.5 class, i.e. *this row's own retiring family*. So if `asia-south1` never serves a 3.x model, the only Gemini it does serve dies on this date and D-127's model leg reopens as a decision (OPERATIONS §2 gate 14). It is not an outage in waiting: Sarvam carries the assistant with a disclosed fallback the whole time (G-6), and the ENDPOINT decision — regional, never global — does not reopen whatever the answer is. Also still owed: the regression run, blocked outside this repo on a Sarvam key and egress (§7 fixtures). |
