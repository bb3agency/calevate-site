...
|----|------|-----------|--------|------------|
| R-01 | ~~**No legal entity decided**~~ **AMENDED (D-461): the entity is DECIDED and this row is now about the REGISTRATIONS.** `docs/legal/LEGAL-OPS-PLAYBOOK.md` §0/§3 (24 Aug 2026, the legal source of truth) fixes it: a **sole proprietor** trading under the product name Calevate — no company, no LLP, no CIN, and the playbook is explicit that incorporating would make it stale. What still blocks outbound is Calevate's own **DLT Telemarketer registration**, which follows **Udyam**. Note the correction the old text also carried: **PE registration is the CLIENT's**, in their own name (playbook §10.4) — we are the TM they bind, and we cannot file a PE for them. | Certain until the TM registration exists | Critical | Udyam (MSME) → DLT TM registration under the proprietor's PAN. Each client takes their own PE registration (₹5,900 first TSP) and binds our TM-ID; we accept the chain. Inbound-only launch is the fallback while pending. |
...
| R-04 | ~~Gemini 2.5 family retirement 16 Oct 2026~~ **CLOSED 19 Aug 2026 by D-410 — the product no longer runs a Gemini model on any surface** | n/a — the model is gone | None outstanding | **This row is kept, struck, because its history is the argument for how the next one should be handled.** It was closed for the extraction leg from the start — Sarvam 105B is free per token and runs that pass permanently (`GEMINI_EXTRACTION_DEFAULT is False`, D-127 G-7), so no post-call path was ever exposed to the date. The other half stopped being a risk and became an OBLIGATION on 16 Aug 2026: D-134 shipped the dashboard AI on `gemini-3.1-flash-lite` specifically to escape this row, D-142's re-search then found no 3.x model reported in `asia-south1` while the 2.5 class was, D-127 would not move the region, so the founder took the model the region served and took the date with it. D-400 then WIDENED the row by moving the in-call leg onto the same identifier, and D-404 removed the last mitigation by making that leg deliverable. **D-410 closes it outright**: both LLM surfaces moved to Azure OpenAI South India, `gpt-4o-mini` by default, and the Gemini model, the date-carrying constant `GEMINI_DEFAULT_LLM_RETIRES`, the CI test that turned red thirty days out and OPERATIONS §2 gate 14b are all deleted. **Nothing is owed against a vendor deadline today.** **What must survive this row's closure, because it is the part that worked:** a vendor retirement was held as DATA in one constant, one gate named the three steps and the wrong answers, and a test failed the build BEFORE the date rather than an incident reporting it after. If a retirement is announced for `AZURE_OPENAI_DEFAULT_MODEL`, that is the mechanism to rebuild — not a calendar reminder, and never a widened runway constant to quiet the test. **What was genuinely PAID for the Gemini decision and is not recovered:** the weeks this row spent as a live obligation, and the migration work D-134 did twice. **What is genuinely BOUGHT:** a default model with no announced end-of-life running against it, on both surfaces, at a lower list price. |
