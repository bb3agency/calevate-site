...
| R-04 | Gemini 2.5 family retirement 16 Oct 2026 | Certain | **Low → now largely NEUTRALISED (D-35/D-36)** | Sarvam 105B is free per token and is the D-36 default LLM, so the migration is no longer forced. Gemini remains a configurable fallback; if used, migrate to 3.x Flash-Lite and re-run the regression suite. |
