...
| R-04 | Gemini 2.5 family retirement 16 Oct 2026 | Certain | **Closed for the extraction leg. LIVE, DATED and ACCEPTED for the dashboard leg AND, since D-400, for the IN-CALL leg — the product now RUNS on the retiring family and has decided to run more of itself on it.** | Sarvam 105B is free per token and runs the extraction leg permanently (`GEMINI_EXTRACTION_DEFAULT is False`, D-127 G-7), so no post-call path was ever exposed to this date, and that half stays closed. **The other half stopped being a risk and became an obligation on 16 Aug 2026.** D-134 shipped the dashboard AI on `gemini-3.1-flash-lite` specifically to escape this row; D-142's re-search then found no 3.x model reported in `asia-south1` — global plus the `us`/`eu` multi-region REP endpoints only — while the 2.5 class IS reported in Mumbai. D-127 does not move the region (that is the residency leg the whole product rests on), so the founder chose the model the region serves and took the date with it: `GEMINI_DEFAULT_LLM` is `gemini-2.5-flash`. **D-400 WIDENED THIS ROW rather than changing it.** The founder moved the IN-CALL LLM leg to the same paid Vertex account and the same `GEMINI_DEFAULT_LLM`, so the identifier that dies on 16 Oct 2026 is now the one two of the three LLM surfaces name. That is a deliberate consolidation and it makes the replacement CHEAPER, not dearer — one constant, one date, one gate — but it also means the day this row comes due is a day the in-call leg is exposed too, if it is live by then. It was not live when D-400 landed (`VERTEX_IN_CALL_CREDENTIAL_DELIVERABLE` was False, D-402), which was the one thing keeping the blast radius where it was — and **D-404 has now removed that mitigation deliberately**: the credential blocker was answered by rotating the bearer on a cron, `VERTEX_IN_CALL_CREDENTIAL_DELIVERABLE is True`, and any deployment holding a GCP project and service account runs the in-call leg on this identifier. The blast radius of this date is therefore as wide as this row's second sentence always said it would be once the leg landed, and the replacement work below is now owed for two live surfaces rather than one. **This row therefore no longer describes something to avoid; it describes work that is owed by a date.** What is owed, and by when: replace the identifier before 16 Oct 2026 — OPERATIONS §2 gate 14b, which names the three steps and the wrong answers. What will remind whoever is here: `calevate_shared.engine.GEMINI_DEFAULT_LLM_RETIRES` holds the date as DATA and `tests/sarvam_model_identifier_test.py::test_the_shipped_gemini_model_has_runway_left` turns CI red on **16 Sep 2026**, thirty days out, with the remedy in the failure message. What it costs if nobody acts: not an outage — every assist 404s, the worker logs `vertex_model_not_served_in_region`, and `assist_capability()` serves the disclosed Sarvam fallback (G-6). What is still NOT known: whether Mumbai serves this identifier at all — `GEMINI_MODEL_CONFIRMED_IN_REGION is False`, one `generateContent` closes it, and the only thing blocking that call is a GCP project. Also still owed: the regression run, blocked outside this repo on a Sarvam key and egress (§7 fixtures). |
