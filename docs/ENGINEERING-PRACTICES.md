...
| `web-check` (frontend gate) | **SHIPPED** (`make web-check` — `pnpm -C apps/web typecheck`, `lint`, `test`; a prerequisite of `make check`, and in CI's frontend job, which also gained `pnpm lint` — previously defined and wired into nothing) — D-53 | The frontend's own behavioural contracts, which `tsc` structurally cannot see: fail-CLOSED vs fail-VISIBLE lookups, verdicts keyed on the server's computed answer rather than a raw status, money as a string that is never parsed, a null cost that renders "we cannot say" and never ₹0. Vitest + jsdom + Testing Library, rendered tests going through the real provider, session and request layer with `fetch` as the only seam. `next build` stays CI-only (too slow for the local gate). It found two live prototype-chain bugs on its first run |
