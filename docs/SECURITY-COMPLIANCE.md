...
| TRAI recording rule | Recording retention | **90-day minimum retention of call recordings** (floor in retention_policies, a DB CHECK on `retention_policies.ttl_days` and `RECORDING_FLOOR_DAYS` in `apply_retention`). ⚠ **THIS CELL SAID "…ON INDIAN INFRASTRUCTURE" AND THE LOCATION HALF IS WITHDRAWN, 22 Aug 2026.** No primary source for it could be reached or found (searched 22 Aug 2026; TRAI's own regulation text is behind an egress-blocked host and secondary sources describe retention periods per sector regulator — RBI 2y, IRDAI 6m — and none imposes a storage location on a telemarketer). It also contradicted this repository's own disclosure: recordings live in Cloudflare R2, which offers **no India-only jurisdiction**, and `/legal/subprocessors` and `/legal/privacy` §8 tell clients so. A rule we cannot cite that we would be breaching if it existed is the worst of both, so the 90-day FLOOR stays (it is enforced in two places and is the conservative number) and the location claim goes to counsel — OPERATIONS §2 gate 37, with S-6. `apps/api/compliance/deletion.py:62` quotes the old wording and is another lane's to correct. |
...
2. **Recording notice** (DPDP-side, a different regime — see §1's table): its own
   ...
   own privacy notice or prior consent artefact — and it does not discharge it.

   ⚠ **THE NEXT THREE SENTENCES USED TO BE WRITTEN AS BUILT AND CONTRADICTED THE ONE
   ABOVE IN THE SAME BULLET.** They read: *"Explicit consent captured for outbound
   recording where required; `calls.consent_recording` + immutable `consent_ledger` row
   with evidence (transcript span). Caller decline ⇒ recording off, call continues,
   ledger row written."* — three lines under "nothing in this codebase can [switch
   recording off]". **It is the SPECIFICATION and none of it is wired.**
   `Call.consent_recording` is on `scripts/check_wiring.py`'s known-unwired list with its
   reason (*"the engine reports no per-call recording consent yet (pilot gate 3)"*);
   `CONSENT_PURPOSES` carries `recording` and nothing writes a row under it; and
   voice-runtime has exactly ONE in-call tool (`opt-out`), so no agent can stop a
   recording mid-call. **Closes at pilot gate 3**, and until then the built behaviour is:
   every call an agent handles is recorded, the truthful ANSWER (§2.0) tells a caller so
   whenever they ask, and a caller who wants the audio gone is an erasure request the
   client runs. `/legal/privacy` §4.1 and `/legal/acceptable-use` §2.6 stated the unbuilt
   half as live to callers and to clients until 22 Aug 2026 and now say this.
...
| Retention limits | retention_policies per category with TTL enforcement job; **enforced defaults** (`scripts/seed.DEFAULT_RETENTION_POLICIES`, and the numbers published at `/legal/privacy` §9): recording **90 days** (TRAI floor), transcript **365 days**, lead **1095 days**, consent_log retained; BFSI clients configurable ≥ regulator minimum. *(F-5: the earlier "default 180 / 24 months" figures were stale and are superseded by these enforced-and-published defaults; the final per-tenant numbers remain a DPA commitment to confirm with counsel — playbook §20. Reconciled 24 Aug 2026, ROADMAP D-462.)* |
...
| Cross-border | **CAUTION, AND IT IS BROADER THAN THIS ROW USED TO SAY.** It read "Bolna call recordings observed on S3 us-east-1" — storage only, and inferred from a URL. Their own published documentation says the WHOLE PLATFORM: *"By default, all Bolna AI services operate in United States (US)-hosted infrastructure, but customers on enterprise plans can choose to have their data processed exclusively in India"* (`bolna-findings/mirror/pages/enterprise/data-residency.md`) and *"By default, Bolna processes calls on infrastructure in the US (AWS us-east-1)"* (`concepts/security.md`). India residency is an **Enterprise-plan purchase nobody has made** (gate 12) — and the part that makes this an engineering fork rather than a procurement item is that their India-routing requirements EXCLUDE BYOK: *"Use Bolna's default provider integrations. Do not connect your own API keys for the transcriber, synthesizer, or LLM providers"*, with the consequence stated outright — *"If you connect your own API keys for any provider (transcriber, synthesizer, or LLM), calls will automatically route through US servers regardless of other configuration settings"* (`enterprise/indian-server-configuration.md`). **BYOK on all three legs is what this product IS** (D-31/D-36/D-410; `engine/bolna.py::_agent_body` sends our own model strings on every leg), so buying the residency option would not by itself move one call. `docs/evidence/bolna-compliance-residency.md` §2 splits the picture into processing / transcripts / recordings / metadata with the quote behind each, and §5 states the fork. **THE SENTENCE THAT USED TO STAND HERE — "Everything the caller says is processed in India" — IS FALSE AT THE ORCHESTRATION LAYER AND IS REMOVED.** What survived that correction was narrower and was still worth having: the MODEL legs were Indian — Sarvam for speech, vendor-sovereign, and Azure OpenAI South India for language — so the inference itself did not leave the country, while the platform that carries the audio, holds it in flight and keeps its own copy of the recording and transcript sits in the US. **THAT HALF IS NOW GONE TOO, AND THIS ROW WITHDRAWS IT RATHER THAN NARROWING IT AGAIN [D-449, 22 Aug 2026].** The language leg is Azure OpenAI in **East US 2**. **There is no longer any India residency claim to make about the caller's words**: the transcript reaches a US model on every turn of every call, as it is spoken. ⚠ **AND SINCE 27 AUG 2026 THE SPEECH HALF IS WITHDRAWN TOO — this sentence read "the speech legs remain Sarvam and Indian".** Sarvam is an Indian COMPANY, which is not a claim about where processing happens: its own privacy policy states personal data *"may be transferred to and processed in countries outside India"*, naming **United States** cloud infrastructure (AWS, GCP, Azure) and analytics providers and **European Union** model and security vendors, with EU SCCs, adequacy decisions and DPAs as safeguards, and its *"Data Localization (Indian Users)"* carve-out storing voice biometric data in India reads as scoped to **Content Studio**, not to Voice Agents / API traffic. **So the AUDIO may leave India on the SPEECH leg, before any language model sees a word** — which is why gate 37(a) (is a voice recording SPDI biometric data?) now bites this leg as well. A second fact from the same reading, because it lands on a different row: **ToS s.17.5 permits Sarvam to use Inputs, Outputs and usage data to TRAIN its models** (per its Privacy Policy and applicable law and, where required, a declinable consent — access to some Offerings may be restricted if declined); it does **not vary by tier**, and **s.6.2** makes a signed order form or enterprise agreement the only instrument that displaces it. We hold none, so `/legal/dpa` cl.2, `/legal/privacy` §6 and `/legal/subprocessors` §3.4 now state the vendor position rather than promising it away. VENDOR-PUBLISHED (Sarvam Privacy Policy "Cross-Border Data Transfers" and its retention/deletion tables; Sarvam ToS v2.0 eff. 29 Jul 2026 ss.6.2/17.5 — read by the founder at `www.sarvam.ai` on 27 Aug 2026 and relayed. ⚠ `sarvam.ai` and `docs.sarvam.ai` remain egress-blocked from this container; this did NOT come from a fetch here). Read the ladder below as a HISTORY that ends in a withdrawal, not as a fourth and thinner formulation of a promise that still stands. The client DPA and `/legal/subprocessors` were corrected to say so on 20 Aug 2026. Since D-127 the language claim is about ENDPOINTS rather than about vendors. Since D-410 it was a claim about a REGION-PINNED RESOURCE, weaker still. **Since D-449 it is not an India claim at all**, and this table says so rather than letting the sentence coast.** The oldest form of the argument was "Sarvam is sovereign, therefore no transcript text leaves the country" (D-36) — a fact about a VENDOR, and no longer available for the language leg. D-127 replaced it with a fact about an ENDPOINT: `asia-south1` in a Vertex URL, nine characters `scripts/check_model_residency.py` could prove from the AST. **D-410 replaced the vendor and the guarantee lost a rung.** Azure OpenAI's shipped endpoint, `<resource>.openai.azure.com`, names no region at all: the region is a property of the RESOURCE, so `AZURE_LOCATION` is an assertion the code makes and a human confirms in the portal (OPERATIONS §2 gate 20), not a string a guard can read. That was a real weakening, taken deliberately for the reasons in D-410, and the honest description of that posture was **configured, guarded and attested — not proved.** **D-449 DOES NOT ADD A FIFTH ADJECTIVE TO THAT LIST. It removes the subject.** The machinery is unchanged and still runs — one spelling of the region, no `Settings` field carrying one, one builder, and a guard that fails the build in BOTH directions on a declared posture — but what it now guards is that the model traffic lands in `eastus2` rather than that it stays in India. A mechanism can be honest about a destination it does not like; what it cannot do is turn a US destination into an India warranty, and no wording in this table should be read as attempting it. Three legs, three reasons. **(1) The in-call stack is Sarvam for SPEECH and, since D-410, Azure OpenAI for LANGUAGE — in South India then, in `eastus2` since D-449.** ⚠ **THIS READ "Saaras STT and Bulbul v3 TTS are unchanged and sovereign BY VENDOR" UNTIL 19 SEP 2026, AND THE TTS HALF NAMES A LEG SARVAM NO LONGER SERVES.** **D-629 (18 Sep 2026) took Sarvam off the TEXT-TO-SPEECH leg entirely.** The two halves must now be read apart, because they carry different data to different companies in different countries. **STT is Sarvam Saaras and is UNCHANGED**: it receives the caller's AUDIO and the raw transcript on every call, and it still runs the first post-call extraction pass. "Sovereign by vendor" was always a statement about the COMPANY and not about where the audio is processed — Sarvam's own cross-border clause permits processing outside India, so Telugu audio and its transcription may leave the country (correction of 27 Aug 2026, above), and that correction stands untouched. **TTS is chosen PER AGENT and is no longer an Indian vendor at all**: `agents/voices.TtsModel` is `Literal["sonic-3.5", "timbre-v2.5"]` — **Cartesia Sonic 3.5** (the Studio rung) or **Gnani Timbre v2.5** (the Clear rung). In practice only Cartesia speaks today, because hard rule 7 admits an operator's attested INVOICE figure and nobody has recorded a Gnani one, so every Gnani voice is refused at the offer seam (`agents/voice_offer.tts_price_is_billable`). **WHAT THE TTS LEG RECEIVES IS NARROWER THAN WHAT THE STT LEG RECEIVES, AND THE DISTINCTION IS LOAD-BEARING FOR THE DPA**: a synthesiser is sent the words the AGENT is about to say — our composed prompt's output — and never the caller's audio and never their transcript. It is still personal data whenever the agent speaks a caller's own name or number back to them, which the prompt makes it do (digit-by-digit read-back), so it is a sub-processor and is named as one below. The LLM leg is region-pinned by configuration: `azure_openai_base_url()` is the only way an Azure endpoint can be constructed in this tree, `AZURE_LOCATION` is the only spelling of the region, no `Settings` field may carry a region, and the builder cannot emit a region other than the declared posture's. **That pin now buys disclosure accuracy, not localisation.** **(2) The first post-call extraction pass is Sarvam and stays Sarvam** (D-127/G-7, `GEMINI_EXTRACTION_DEFAULT is False`), because it is the ONE path that must see raw text: `workers/pipeline.py` hands the extractor the raw turn one line after computing the redacted one, deliberately, since a CRM callback-number field needs the digits. D-400 did not move it and D-410 does not either. **(3) The dashboard assistant — ⚠ AND BOTH HALVES OF THIS SENTENCE WERE OVERTAKEN, CORRECTED 1 SEPT 2026 (LEGAL-SURFACE F-16).** It read *“is Azure OpenAI and nowhere else — `eastus2` since D-449 — over the REDACTED copy plus tenant-authored config, never raw personal data (D-127/G-2)”*. **On the PROVIDER half**: the leg is Azure by default and may also run on **Google (Gemini)** for a tenant whose own model is a Gemini one, and only while an operator attestation stands that our account with that provider is on a tier that does not train on submissions (`agents/llm_models.DASHBOARD_NEEDS_DATA_USE_ATTESTATION`); **`openai` is BARRED from this leg** and is fail-closed in `DASHBOARD_TERMS_UNREAD` because nobody has read that vendor's data-use position from a primary source — an unread position is not a permission. The disclosed **Sarvam** fallback (G-6/G-7) is unchanged and answers in prose with NO tools. So the assistant leg is NARROWER than the in-call leg, not equal to it, and `/legal/subprocessors` said the opposite about OpenAI until this correction. **On the CONTENT half**: since the copilot gained read tools (`apps/api/copilot/tools.py`) the leg carries what the user typed plus lead NAMES and statuses, recent calls with their already-redacted summaries, campaign and agent names, counts, and the tenant's own knowledge passages — not “tenant-authored config”. **G-2 survives intact and is what makes that acceptable**: every tool result goes through `redact()` before it reaches the model, no raw transcript and no extraction payload is returned by any tool, and a number arrives as `[phone ••NN]`; permission is checked in code (`rbac.role_has`) rather than in the prompt, and each tool opens its own short `tenant_session` so RLS answers tenancy. A FOURTH surface joins the same leg: `apps/workers/copilot_memory.py`, an hourly cron that reads a run of past copilot exchanges back to the **Azure** leg to distil durable business facts, metered through `record_ai_assist_usage`. And the copilot now PERSISTS — `copilot_memories`, 180 days, per user, redacted on write, deleted outright at tenant offboarding; the retention row above carries the period and LEGAL-SURFACE §2.2 carries the category. Every G-rule D-127 wrote survives the vendor change AND the region change unaltered and binds Azure exactly as written; G-1..G-7 are the reason this leg is the one least disturbed by D-449, because redaction happens before the call and the destination was never what made it safe. **TWO THINGS CAN INVERT THIS POSTURE AND NEITHER IS VISIBLE FROM AN ENDPOINT, WHICH IS WHY BOTH ARE HARD GATES.** First, the resource could simply be in another region — gate 20, one human, one portal reading, filed in `docs/evidence/`. Second, and this is the trap: **Azure's DEFAULT deployment type is GLOBAL, which routes requests to capacity anywhere in the world.** Regional Standard is what keeps processing in the resource's region, it costs roughly 5–10% more (published examples up to +12% and +20%), and a Global deployment inside the resource passes every automated check in this repository while making the region named in the DPA unenforceable. That is gate 20c, and the premium is a payable cost of the posture rather than an accident. **Since D-449 the premium buys an accurate sub-processor disclosure rather than an India warranty — and it is the ONLY enforceable property the posture has left, so "it is in the US anyway" is not a reason to accept Global: Global routes to capacity anywhere in the world, which is not a US promise either.** **Every alternative was once refused for moving the caller's words out of India, and D-449 spends that ground — so the refusals are restated on grounds that still discriminate.** **OpenAI direct's India data residency covers STORAGE AT REST ONLY — inference still runs in the US, and in-region GPU inference exists only in the US and Europe. For a phone call the transcript IS the inference input**, so the residency that product offers is residency for the wrong half. That sentence is still true and is kept because it is the sharpest statement of what residency means for a voice product; **but it no longer DISQUALIFIES anything, because we have stopped asking for Indian inference (D-448 refused both direct APIs on exactly this ground, and D-449 spent it).** Azure is retained on grounds that survive the move: an enterprise DPA, modified abuse monitoring, and deployment-level control of which model version runs and when it retires — which `scripts/check_model_lifecycle.py` already consumes as a build gate — against a migration cost already specified. DeepSeek is China-hosted and disqualified for Indian callers' transcripts outright. Krutrim is GPU IaaS, not a managed API, and would mean running our own model server. The AI Studio / Gemini Developer API was disqualified on the same axis before Gemini left the product at all (D-401/D-406/D-407), and on the free tier Google states it uses submitted prompts and responses to improve its products with human reviewers able to read them — a disclosure a Processor holding an SMB's callers' words cannot make. Because a region is a few characters in a string, none of this is left to review: `scripts/check_model_residency.py` runs in `make guardrails` and fails the build on any model endpoint constructible outside the one builder, on a region that is not a frozen `Final` constant or that is reachable from console-editable config (a residency posture invertible from a web form at 3am is not a posture — the doctrine D-95 §4 applies to `APP_ENV`), and on any attempt to spell the region twice. **What that guard CANNOT see, stated here because the gap is the point**: which region the Azure resource was created in, and which deployment type it uses. Those are gates 20 and 20c and they are attestations, not proofs. **The client-facing documents now state the assistant leg as it is (1 Sept 2026):** `/legal/privacy` §3.2/§6/§8/§9/§12.4, `/legal/dpa` cl.2, cl.8, Annex A and Annex B.2, `/legal/terms` §4/§5, and `/legal/subprocessors` — four corrected rows plus a new §3.5. Privacy, Terms, the DPA and the register all step to **revision 3, material**, because a new category of stored personal data and a new processing purpose is what `material=True` is for. **One thing is disclosed and not yet mechanised** and it is tracked as LEGAL-SURFACE FOLLOW-UP-12: `execute_deletion_request` has NO arm over `copilot_memories`, so a data principal's erasure does not search a store that can hold their first name for up to 180 days (`redact()` catches identifiers, not proper nouns — `copilot/memory.py` says so itself). `/legal/privacy` §12.4 states that limit and marks it as the one not yet on the certificate; `ERASURE_LIMITATIONS` must gain it, or the erasure must gain the search arm. Adding it is a WIDENING of the limitations text, which this section permits. **Client DPA sub-processors, named**: Bolna (orchestration; **United States by default AND in practice, because BYOK forecloses their India routing** — see the CAUTION above), Exotel/Vobiz/Plivo (telephony), Sarvam AI (**SPEECH-TO-TEXT**, the permanent post-call extraction pass, and the disclosed dashboard-assist fallback — an INDIAN COMPANY whose own privacy policy permits processing outside India, including on US cloud infrastructure and with EU model and security vendors; the register's Location cell says exactly that since 27 Aug 2026 and no longer says "India". ⚠ **THIS CELL SAID "speech" UNQUALIFIED UNTIL 19 SEP 2026, WHICH OVERSTATED IT**: D-629 withdrew the Sarvam synthesis leg, so Sarvam transcribes and no longer speaks), **Cartesia (TEXT-TO-SPEECH, the Studio rung — a UNITED STATES company; it receives the words the agent speaks, never the caller's audio or transcript)**, **Gnani (TEXT-TO-SPEECH, the Clear rung — an INDIAN company; published but NOT SELLABLE, because hard rule 7 admits only an operator-attested invoice figure and none has been recorded, so no call has ever been synthesised on it)**, **Microsoft Azure (Azure OpenAI, East US 2 — South India until D-449), which since D-410 covers BOTH LLM surfaces — the in-call leg and therefore raw caller speech, and the dashboard assistant over redacted data**, and **Google Cloud, which remains a sub-processor for ONE thing only: Google Sheets lead delivery (D-23)**, where a client shares their own document with our service account and lead rows are appended to it. That narrowing was checked rather than assumed — Sheets holds its own credential (`google_sheets_service_account_json`) and is unaffected by the LLM move — and it is a DPA change in both directions: Google Cloud drops out of the transcript path entirely, and Microsoft enters it. The scopes differ and the DPA must say so: the dashboard leg sees `text_redacted` and tenant-authored config, the in-call leg sees the conversation as it happens, and the Sheets leg sees the extracted lead row including, on a per-endpoint opt-in, the phone number. **A TENANT SWITCHING AN AGENT'S MODEL IS NOT A RESIDENCY CHANGE, AND THIS ROW SAID IT WAS UNTIL D-454 SHIPPED THE PICKER [22 Aug 2026].** The old sentence — *"A tenant switching an agent's LLM leg is still a residency change and not a config tweak"* — was written when only an operator could move that value. `agents.llm_model` -> `organizations.default_llm_model` -> `Settings.azure_openai_model` is now a CLIENT-facing three-level choice (`agents/llm_models.py::resolve_llm_model`). **⚠ AND THE REASSURANCE THAT USED TO FOLLOW THAT SENTENCE IS ITSELF NOW WITHDRAWN.** It read: *every model in `AZURE_OPENAI_MODELS` is served by the same vendor, from the same `azure_openai_resource`, through the same `azure_openai_base_url()`, under the same human-attested deployment type — it moves a PRICE and a QUALITY, not a destination.* That was true while one leg was on offer and is FALSE now that three are: the offered set spans `azure_openai` (East US 2, attested), `openai` (the `us` residency host, provable from the hostname) and `google` (the Gemini Developer API, which has NO region to request at all — its SDK raises before a packet leaves the machine). **A client choosing a model is therefore also choosing which provider handles the language leg and where it runs**, which is a sub-processor change and not a config tweak. It is disclosed as one: `/legal/dpa` and `/legal/subprocessors` were corrected to say the single-vendor, single-region promise about that leg is WITHDRAWN, not narrowed — the same move D-449 made about India, one level down, and for the same reason (a promise kept alive with qualifiers is worse than one retracted). What did NOT change is the shape of the guarantee that remains: the set of providers this code may reach AT ALL is a closed `Literal` in `calevate_shared.engine` moved only by a reviewed commit, never by a control on any screen, and the speech legs and the first extraction pass are still Sarvam — an Indian VENDOR, and since the 27 Aug 2026 correction above, NOT an India-processing claim. **What IS a residency change, and it is a setting rather than a commit:** `azure_openai_resource` — the region is a property of the RESOURCE, `platform_config.py`'s own `AppliesRule` says *"a resource in the wrong region is a residency change no code here can detect"*, and it is an editable field in the ops console's *Language model* group. That is what gate 20 is a reading OF, and it is why the client DPA no longer says no setting can move the region. This is a live differentiator: Outpero's privacy policy admits "some providers may process data on servers located outside India" (evidence/outpero-teardown-aug2026.md §9b) |
...
| Cross-call caller memory — **BUILT AND OFFERABLE, OFF BY DEFAULT, BEHIND A PER-TENANT ATTESTATION (D-513)** | A caller who is REMEMBERED ACROSS CALLS is a materially different privacy proposition from one who is not, and D-506 held the feature shut on three questions. All three are now answered. **(a) THE AGENT SAYS IT ALOUD.** A third sentence, `agents.caller_memory_notice_line`, NOT NULL and non-blank like the AI sentence, composed into the opening by `compose_opening_line` — and with **NO SWITCH OF ITS OWN**: it is spoken exactly when `caller_memory_enabled` is true, so "remembers a caller and does not say so" is not a constructible state. The two notices in §2.1/§2.2 are independently switchable (D-163) because their obligations hold whatever this product is configured to do; this one exists only because a switch we record is on, which is why binding it is right rather than merely safe. Written notice alone was rejected: `compliance/caller_notice.py`'s draft reaches the people who came through the client's own funnel, and an INBOUND caller — the product — has visited no website. **(b) TREATED AS POTENTIALLY SPDI, WITH THE SYSTEM NOT ALLOWED TO DECIDE PER FACT.** The SPDI Rules 2011 list is EXHAUSTIVE and includes "physical, physiological and mental health condition" (Rule 3), so "asked about IVF pricing" is, on a clinic, an inference about exactly that; SPDI Rule 5(1) wants consent IN WRITING, which a phone call cannot give. `caller_memory.SPDI_REFUSED_VERTICALS` refuses the write for those tenants however the switch is set — a known-weak proxy (`vertical_template` is only what a tenant STARTED from) kept because it is the only structured signal and it errs toward refusing. **(c) 180 DAYS, `delete`, ITS OWN `retention_policies` CATEGORY** with its own sweep arm, rather than the tenant's 365-day transcript clock: a memory whose purpose is to outlive the call must not inherit the call's period. ⚠ **WHICH REGIME GOVERNS IS REPORTED, NOT VERIFIED**: DPDP §§5-6 and §44(2)'s repeal of the SPDI Rules are reported to commence 13 May 2027 (18 months from the 13 Nov 2025 gazette), so SPDI is still operative — but `meity.gov.in` is egress-blocked from this container and that schedule was not read here. The answers above are the conservative side of both regimes. **THE TWO MISSING PIECES ARE NOW BUILT (D-513), AND THIS ROW USED TO END "WHAT IS STILL MISSING: the distillation producer, and any route that flips the switch".** **(d) THE PRODUCER** is `workers/caller_memory_distil.py`, an hourly cron whose fan-out STARTS from `agents.caller_memory_enabled`, so a deployment where nobody switched it on makes zero model calls. It reads `transcript_turns.text_redacted` and never the raw column, is bounded at three facts per call and 240 characters per fact, and every fact passes `within_scope` — a coarse refusal of money figures and clinical words that holds the founder's scope decision when the prompt does not, above which `SPDI_REFUSED_VERTICALS` still refuses the whole feature. Idempotency is `calls.caller_memory_state`, four values, stamped in the same transaction as the rows it describes, because `source_call_id` can say "produced a fact" and can never say "read and owed nothing". It is METERED TO THE CLIENT (hard rule 7), so it appears on their bill and **their spend cap can stop it**. **(e) THE ROUTE** is `PATCH /v1/agents/{id}/caller-memory`, `org:manage` — the client's own decision, audited, republished to the voice platform in the same transaction so the spoken sentence and the column cannot disagree. Switching it on for the first time requires an ATTESTATION recorded on `organizations.caller_memory_attested_at`: the per-tenant instrument (b)'s proxy describes itself as standing in for. The proxy stays ABOVE it — a clinic is refused whatever it attests, because a vertical is a record and an attestation is a claim. **(f) THE INBOUND LEG**, `GET /v1/engine/caller-data/{engine}`, is how a caller who RANG US is recognised: the engine holds the number, so it fetches from us at call setup and injects the answer into that call's instructions. Bearer token, constant-time compare, and it FAILS OPEN on everything but authentication. Still held by `tests/caller_memory_erasure_guard_test.py`, which refuses to let the scope be live without a `DERIVED_COPIES` entry and an erasure arm (a DPDP erasure scrubs a call in place, so the `ON DELETE CASCADE` on `call_id` never fires). |
...
- **Access**: same class as a raw transcript and gated the same way — `calls:read_raw`
  ...
  `staff` see the delivery record and never the body. An impersonating admin — of EITHER
  tier since the founder's correction to D-457 moved `calls:read_raw` into the normal one
  — sees the record freely and reaches the body only by asking for it, having passed a
  second factor at the impersonation door (D-210) and leaving the audit row this bullet
  names. (This line used to put an impersonating operator on the `staff` side of that
  boundary; that was true of the old split and is not true now.)
...
- **§1 (TRAI recording rule) and §4's own retention row** record a **90-day minimum
  retention** of call recordings — a floor the codebase treats
  as binding in two independent places: a DB CHECK on `retention_policies.ttl_days`, and
  `apply_retention` clamping every recording TTL to `RECORDING_FLOOR_DAYS = 90`. (The
  words *"on Indian infrastructure"* stood here too and are withdrawn with §1's — see
  that row. The DURATION is what this decision turns on, and it is unaffected.)
...
- §4's retention row above now states the **enforced-and-published defaults**: **recording
  ...
  2555 days, engine_payload 90 days, kb 365 days, copilot_memory 180 days, caller_memory 180 days (D-507) — the rows a new tenant gets, the rows the
...
- `copilot_memory` (migration d4a9c17e6b02) was likewise never part of the divergence — it
  did not exist when this section was written. It is the in-app assistant's memory: what a
  client's own STAFF asked the copilot about their own console, redacted on write, plus the
  durable business facts a background worker distils out of it. **180 days**, shorter than
  the transcript clock, because nothing depends on these rows: they are working context,
  regenerated by use, and the client bought none of them. `action` is not read on this
  category either, for the `kb` reason — a memory row is free prose, so a blanked one would
  still be recalled into a prompt while carrying nothing. It is a per-tenant default a
  client may change, and it is deliberately NOT in the CALLER-facing notice generator
  (`compliance/caller_notice._CATEGORY_LABELS`): a client's callers are not the subject of
  this data, so printing its period in a notice addressed to them would describe the wrong
  people's records. Offboarding deletes every row unconditionally
  (`retention.execute_tenant_erasure`), which is the arm that carries the residual a
  redaction pass cannot: `redact()` recognises identifiers, not proper nouns.
...
- RBAC: admin{superadmin,operator}; client{owner,staff}. The two ADMIN tiers are managed
  ...
  - **THE TIERS DIFFER BY FOUR PERMISSIONS AND NOTHING ELSE**, named in
    `core/rbac.SUPERADMIN_ONLY_PERMISSIONS` and asserted as a set identity against
    `SUPERADMIN_PERMISSIONS` in `tests/admin_operators_test.py` — so a permission added to
    the `Permission` type cannot land on a side nobody chose. The four are
    `platform:secrets` and `platform:config` (the ops config panel, where every vendor
    credential and every platform-wide setting is installed — §10's risk acceptance rests
    on the first being held by fewer people than anything else), `admin:operators` (the
    role table itself: a normal admin who could edit it could grant themselves the other
    three) and `ops:manage` (the global kill switches — and, because two routes read it as
    the superadmin marker rather than declaring it, tenant erasure and the raising or
    removal of a client's spend ceiling ride on it too).
  - **EVERYTHING ELSE IS ORDINARY SUPPORT WORK AND BOTH TIERS HOLD IT**, including
    `calls:read_raw` and `leads:dispatch`, which the founder moved into the normal tier
    when they drew the boundary. That is a real widening and it is stated rather than
    implied: a normal admin in a view-as session can read an unredacted transcript, play a
    recording, take the unmasked CSV export, produce a data-subject export and open a
  ...
  - **"There will only be one super admin" is not enforced, deliberately.** There is no
    ...
    outside `APP_ENV=local`, and the only statement that updates `admin_users.role` sits
    behind the role it would restore — so a hard cap would make the role
    non-transferable and leave `psql` as the only recovery. What holds the count at one is
    the narrow default role, `admin:operators`, a step-up bound to the role, and a ledger
    row. `apps/api/authn/operators.py` carries the full argument and the residual risk.
