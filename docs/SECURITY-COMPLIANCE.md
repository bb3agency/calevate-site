...
| Cross-call caller memory — **NOT BUILT, and it cannot be switched on under the notices this product gives today (D-506)** | A caller who is REMEMBERED ACROSS CALLS is a materially different privacy proposition from one who is not, and DPDP §5 with Rule 3 wants the itemised list before the processing. Neither channel says it: the two spoken notices (§2.1/§2.2) are about being an AI and being recorded, and `compliance/caller_notice._INHERENT` itemises the number, the recording, the transcript, the summary and the call times — a caller reading a client's generated notice in full still would not learn that the agent remembers them. So the gate is a NOTICE change and not a control: **(a)** does a written notice suffice for an INBOUND caller who never saw the client's website, or must the agent SAY it (a third spoken obligation beside the two D-163 separated); **(b)** is a distilled fact such as "asked about IVF pricing" health data, which the SPDI Rules treat differently from DPDP and which gate 37(a) already has open for the recording itself; **(c)** what retention period may a client agree to for it. All three are counsel's and the founder's. Until then the per-agent switch does not exist and the scope is a value in `caller_chunks.subject_kind` that nothing writes — held open by `tests/caller_memory_erasure_guard_test.py`, which also refuses to let the scope become live without a `DERIVED_COPIES` entry and an erasure arm (a DPDP erasure scrubs a call in place, so the `ON DELETE CASCADE` on `call_id` never fires). |
...
- §4's retention row above now states the **enforced-and-published defaults**: **recording
  ...
  2555 days, engine_payload 90 days, kb 365 days, copilot_memory 180 days — the rows a new tenant gets, the rows the
