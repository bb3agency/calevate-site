...
- Two auth realms (admin vs client), separate cookies/domains; MFA mandatory on admin.
  ...
  - **Step-up (`X-Confirm-Action`) is a SEPARATE control and is retained**, not replaced:
    ...
    MFA. Requiring a *fresh* second factor for high-risk actions is the named next step and
    needs a browser reverification flow — OPERATIONS §8. It is ours to build now rather
    than a vendor's feature to enable: `mfa_verified_at` is the carrier and no route reads
    its age yet (AUTH-MIGRATION §11).
