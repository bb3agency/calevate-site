...
| Cross-border | CAUTION (D-31): Bolna call recordings observed on S3 us-east-1; their Enterprise tier offers full India data-residency (audio, transcripts, logs, in-India inference) — residency posture must be pinned in the Bolna contract and disclosed in the client DPA until then. **Everything the caller says is processed in India, and since D-127 that is a claim about ENDPOINTS rather than about vendors.** The older argument was "Sarvam is sovereign, therefore no transcript text leaves the country" (D-36). That sentence is no longer available — a Google endpoint is in the product — and the guarantee it bought survives only because it has been replaced by a narrower claim that is *enforced* instead of asserted: **every model endpoint this repository can reach is pinned to an Indian region.** Three legs, three reasons. **(1) The in-call stack is Sarvam for SPEECH and, by D-400, Gemini on paid Vertex AI `asia-south1` for LANGUAGE.** Saaras STT and Bulbul v3 TTS are unchanged and sovereign by vendor. The LLM leg is not: D-36's "Sarvam is sovereign, therefore no transcript text leaves India" is now gone from the leg that carries the caller's actual voice, and its replacement is the same one D-127 made for the dashboard — a claim about an ENDPOINT, enforced rather than asserted. It is enforced TWICE here, because the two enforcements see different things: `check_model_residency` proves every Google model URL WRITTEN in this tree names Mumbai, and `ModelConfig`'s validator proves the VALUE does — which is the case that matters when a base URL travels out of our configuration into a third party's agent object, and which that guard says in its own docstring it cannot see. **LIVE since D-404**: `VERTEX_IN_CALL_CREDENTIAL_DELIVERABLE is True`. The one-hour-bearer blocker was answered by ROTATION rather than by a proxy — a 12-hour token minted through `generateAccessToken` and replaced every 4 hours by `apps/workers/vertex_credential.py`, pushed into the engine's credential store through a normalized port. **The residency consequence is the whole reason that was worth building**, and it is worth stating in this table rather than in an engineering note: **every easy alternative moves the caller's words out of India.** A Vertex API key forces the GLOBAL endpoint (Google: "Don't use the global endpoint if you have ML processing requirements, because you can't control or know which region your ML processing requests are sent to"), Vertex Express Mode is global by construction with no `projects` or `locations` segment at all, and Bolna's first-party "Google Gemini" provider is the AI Studio Developer API on a global host — no region pinning exists on that product, paid or free, so paying removes the training-terms objection and leaves the residency one standing (D-401). **A rotating credential is not the inconvenient way to do this; it is the only way that keeps the claim above true.** No code path can reach any of the alternatives, and D-405..D-407 record each with its reason. **(2) The first post-call extraction pass is Sarvam and stays Sarvam** (D-127/G-7), because it is the ONE path that must see raw text: `workers/pipeline.py` hands the extractor the raw turn one line after computing the redacted one, deliberately, since a CRM callback-number field needs the digits. **(3) The dashboard assistant is Gemini on Vertex AI `asia-south1` (Mumbai) and nowhere else**, over the REDACTED copy plus tenant-authored config — never raw personal data (D-127/G-2). The MODEL on that leg is `gemini-2.5-flash`, chosen over a longer-lived 3.x model precisely because the region is not negotiable and no 3.x model is reported in Mumbai — a residency constraint deciding a model choice, which is the correct direction for it to run, and the reason the 16 Oct 2026 retirement is now a dated obligation (BRD R-04, OPERATIONS §2 gate 14b) rather than a reason to reach for a global endpoint. The REGION's residency property is the part of this that is verified as far as this environment can verify anything: `asia-south1` appears in Vertex's own ML-processing-location table (with Tokyo, Singapore, Sydney, Seoul), and Google states ML processing occurs in the region a request is made in for the regions that table lists. What is NOT verified is the MODEL — `GEMINI_MODEL_CONFIRMED_IN_REGION is False`, and it stays False even though the 16 Aug 2026 search now places 2.5 Flash in Mumbai, because a summary of a page this environment cannot open is not a 200 (OPERATIONS §2 gate 14) — and the distinction matters here rather than only in OPERATIONS, because the failure mode of an unserved model is a 404 and a disclosed Sarvam fallback, while the failure mode of an unpinned region would be a residency breach. The guardrail below is what keeps those two failures from ever swapping places: the tempting fix for a 404 is `locations/global`, which Google states gives the caller no control over where processing happens, and `check_model_residency` fails the build on it. The AI Studio / Gemini Developer API is **disqualified, not deprioritised**: it names no region in its host, and on the free tier Google states it uses submitted prompts and responses to improve its products with human reviewers able to read them, which is a disclosure a Processor holding an SMB's callers' words cannot make. Because a region is nine characters in a string, this is not left to review: `scripts/check_model_residency.py` runs in `make guardrails` and fails the build on a global Google model host in any URL literal, on any `*-aiplatform.googleapis.com` literal not carrying `asia-south1`, on a region that is not a frozen `Final` constant or that is reachable from console-editable config (a residency posture invertible from a web form at 3am is not a posture — the doctrine D-95 §4 applies to `APP_ENV`), and on a `locations/…` segment interpolating anything else. **Client DPA sub-processors, named**: Bolna (orchestration; residency per contract, see the CAUTION above), Exotel/Vobiz/Plivo (telephony), Sarvam AI (speech and language, India), and **Google Cloud — Vertex AI, `asia-south1`, India**, which since D-400 covers the IN-CALL LLM leg as well as the post-call dashboard assistant, and therefore raw caller speech as well as redacted data. That widening is a DPA change and not a footnote: the dashboard leg sees `text_redacted` and tenant-authored config, while an in-call leg sees the conversation as it happens. The first post-call extraction pass still sees raw text and still runs on Sarvam, permanently (`GEMINI_EXTRACTION_DEFAULT is False`, D-127 G-7 — D-400 does not move it). A tenant switching an agent's LLM leg is still a residency change and not a config tweak; what has changed is that the destination can no longer be a global endpoint. This is a live differentiator: Outpero's privacy policy admits "some providers may process data on servers located outside India" (evidence/outpero-teardown-aug2026.md §9b) |
