...
| `vertex_llm_credential_refresh_failed` | WORKER_STALL | The in-call LLM bearer did not rotate (D-404). It is a wasting asset: when it expires, **every in-call model turn for every client gets a 401 and the agent goes silent mid-call**, with nothing else in the platform reporting it. | `runbooks/vertex-llm-credential.md`. **You have at least 8 hours** — the token lasts 12 and is replaced every 4, so the first page is a deadline, not an outage. The `detail:` line names which of the six arms failed; §1 maps each to its fix. Six arms share one code because they differ in cause, not consequence. |
| `engine_credential_not_replaced` | CORE_LOGIC | The voice platform APPENDED the rotated LLM credential beside the superseded one instead of replacing it, so it now holds several under one name and chooses between them itself. The leg's health has stopped being a function of anything we do. | `runbooks/vertex-llm-credential.md` §5. Remove the stale entries in the vendor console and re-run the rotation. This also answers an open vendor question: their `POST /providers` documents `added` with no `updated`, so replace-vs-append was never written down — if this fires, it is append. |
